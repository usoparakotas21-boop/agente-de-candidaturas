...
Campos de upload, senhas, caixas de seleção, perguntas abertas, campos ocultos e campos que já têm valor são ignorados. Selects só são alterados para cidade/estado quando um valor ou rótulo de opção coincide exatamente com a localização do perfil. A extensão não escolhe salário, respostas de elegibilidade ou consentimentos, nem envia uma candidatura.

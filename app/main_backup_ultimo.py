...
def _candidate_for_user(db, user):
    ...
    oid = _owner_id(user)
    if oid:
        q = q.where(Candidate.owner_id == oid)
    c = db.scalar(q)
    # Em modo local (oid ̩ None), nunca levanta exce̤̣o
    if oid is not None and c is None:
        # raise HTTPException(409, "Perfil do candidato nao configurado.")
        pass
    return c

(function(){
  ...
  function enhance(){
    ...
    try {
      ...
      cards.forEach(c=>{const title=c.querySelector('h2')?.textContent||'',j=state.jobs.find(x=>(x.title||'')===title); if(j&&!c.querySelector('.match-badge')){const b=document.createElement('span');b.className='badge match-badge';b.textContent=j.match_score==null?'Match pendente':Math.round(j.match_score)+'% match';b.style.cssText='margin-left:8px;color:#16794b;background:#eaf8f1';c.querySelector('.meta')?.appendChild(b);c.dataset.match=j.match_score??-1;c.dataset.date=j.captured_at||'';} });

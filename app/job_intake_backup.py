...
def _extract_location(lines: list[str], text: str) -> str:
    ...
    city_state = re.search(
        r"\b([A-ZÁÉÍÓÚÂÊÔÃÕÇ][\wÀ-ÿ' -]{2,40})\s*[/,-]\s*([A-Z]{2})\b",
        text,
    )
    if city_state:
        return f"{city_state.group(1).strip()}/{city_state.group(2)}"
    city_country = re.search(
        r"\b([A-ZÁÉÍÓÚÂÊÔÃÕÇ][\wÀ-ÿ' -]{2,40})\s*,\s*Brasil\b",
        text,
    )
...
def parse_job_text(raw_text: str, source: str = "texto") -> dict[str, Any]:
    if len(raw_text or "") > MAX_INTAKE_CHARS:
        raise ValueError("O conteudo da vaga excede 80 mil caracteres.")
    text = _clean_text(raw_text)
    if len(text) < MIN_INTAKE_CHARS:
        raise ValueError("O texto possui poucas informacoes para identificar uma vaga.")

    lines = text.splitlines()
    url = _extract_url(text)
    title = _labeled_value(lines, ("Cargo", "Vaga", "Posicao", "Oportunidade"))
    if not title:
        title = _fallback_title(lines)
    title = _extend_title(lines, title)
    company = _labeled_value(lines, ("Empresa", "Company", "Organizacao"))
    if not company:
        company = _fallback_company(lines, title, url)

    fingerprint_source = url.casefold() if url else _normalized(text)
    external_id = "intake-" + hashlib.sha256(
        fingerprint_source.encode("utf-8")
    ).hexdigest()[:24]

    return {
        "source": (source or "texto").strip()[:50],
        "external_id": external_id,
        "company": company[:200],
        "title": title[:200],
        "location": _extract_location(lines, text)[:200],
        "modality": _extract_modality(lines, text)[:50],
        "salary": _extract_salary(lines, text)[:100],
        "url": url[:1000],
        "description": text,
    }

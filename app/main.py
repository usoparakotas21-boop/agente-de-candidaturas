...
def _page(path: Path) -> HTMLResponse:
    ...
    if path.name == "dashboard.html":
        ...
        nav = nav[:style_end]
        nav += '<style>#newJobButton{display:none!important}.hero-actions{display:grid;grid-template-columns:1fr 1fr;width:100%}.hero-actions button{width:100%}@media(max-width:700px){main{margin:24px 12px 60px}.metrics{grid-template-columns:1fr}.queue-summary{grid-template-columns:1fr 1fr}.hero{display:block}.hero-actions{margin-top:20px}.queue-actions{gap:8px}.queue-actions button{min-height:42px;padding:9px 11px}}</style>'
    ...
    if path.name == "security.html": extra = '<script src="/static/security-enhance.js"></script>'

...
@app.get("/")
def root(): return {"agente": "Agente de Candidaturas", "candidato": "Paulo Henrique", "status": "online", "version": "0.23.0", "dashboard": "/dashboard"}
    ...
    try:
        ...
        if existing:
            ...
            if req.reprocess_existing:
                existing.source = parsed["source"]; existing.company = parsed["company"]; existing.title = parsed["title"]; existing.description = parsed["description"]
                for f in ("location", "modality", "salary", "url"):
                    if parsed[f]: setattr(existing, f, parsed[f])
                if req.auto_analyze:
            ...
            if analysis is None: _apply_decision(app, c, None)
            db.commit(); db.refresh(existing); db.refresh(app)
            return {"status": "VAGA_ATUALIZADA" if req.reprocess_existing else "VAGA_JA_EXISTIA", "duplicate": True, "updated": req.reprocess_existing, "job_id": existing.id, "application_id": app.id, "application_status": app.status, "company": existing.company, "job_title": existing.title, "location": existing.location, "modality": existing.modality, "url": existing.url, "analysis": analysis}
        data = dict(parsed); data["external_id"] = ext_id
        job = Job(owner_id=oid, **data)
        db.add(job); db.flush()
        c = _candidate_for_user(db, user)
        app = _ensure_app(db, job, c)
        ...
        if req.auto_analyze:
            ...
            analysis = arts["analysis"]
            _save_analysis(app, analysis, c)
            _advance_app(db, app, "ANALISADA", "Vaga captada e analisada.")
        else:
            _apply_decision(app, c, None)
        db.commit(); db.refresh(job); db.refresh(app)
        return {"status": "VAGA_CAPTADA", "duplicate": False, "job_id": job.id, "application_id": app.id, "application_status": app.status, "company": job.company, "job_title": job.title, "location": job.location, "modality": job.modality, "url": job.url, "analysis": analysis}
    except: db.rollback(); raise
    finally: db.close()

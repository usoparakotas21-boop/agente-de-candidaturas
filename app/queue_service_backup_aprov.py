...
def list_items(
    ...
    Retorna (items, total).
    """
    query = session.query(QueueItem).filter(QueueItem.owner_id == owner_id)
...
def get_summary(session: Session, owner_id: str) -> dict:
    ...
    ).filter(QueueItem.owner_id == owner_id).group_by(QueueItem.decision).all()

    # Contagem por status para REVISAR
    revisar_pendente = session.query(func.count(QueueItem.id)).filter(
        and_(
            QueueItem.owner_id == owner_id,
    ...
    descartar_pendente = session.query(func.count(QueueItem.id)).filter(
        and_(
            QueueItem.owner_id == owner_id,
    ...
    expirado_total = session.query(func.count(QueueItem.id)).filter(
        and_(
            QueueItem.owner_id == owner_id,

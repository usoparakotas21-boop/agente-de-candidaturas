...
As APIs oficiais encontradas não são credenciais gerais para candidaturas em portais de terceiros. O [LinkedIn Apply Connect](https://learn.microsoft.com/en-us/linkedin/talent/apply-connect/create-configure-customer-application?view=li-lts-2026-01) é restrito a desenvolvedores aprovados, com acordo e certificação para obter a chave de produção; o [Indeed Send Candidates API](https://docs.indeed.com/send-candidates-api/) exige integração de parceiro e segue os termos do programa; o [Lever Apply to a posting](https://hire.lever.co/developer/documentation) requer uma chave da conta empregadora que tenha acesso ao endpoint; e o [Create Application da Gupy](https://developers.gupy.io/docs/creating-an-application) usa token de recrutamento da empresa, gerado por administrador/master e com permissões da vaga. Na Gupy, candidato inserido manualmente recebe um convite por e-mail e precisa aceitá-lo; isso não é submissão geral de candidaturas a empregadores desconhecidos. Portanto, não existe uma credencial do usuário da Candidatura Certa que habilite enviar para todos os empregadores ou para os nove portais escolhidos. Cada conector precisa de adesão/autorização da empresa ou parceria aplicável, token de escopo mínimo guardado no servidor e consentimento do candidato para cada destino e vaga.

O LinkedIn declara que não permite software ou extensões de terceiros que raspem, alterem a aparência ou automatizem atividade no site ([política oficial sobre atividade automatizada](https://www.linkedin.com/help/linkedin/answer/a1340567/automated-activity-on-linkedin?lang=pt-BR)); Jobbol veda bots, scripts e extensões para acessar ou interagir com a plataforma sem autorização expressa ([termos de candidato](https://candidatos.jobbol.com.br/termos-uso)); e os termos da Glassdoor proíbem introduzir software/agentes automatizados sem autorização expressa ([termos oficiais](https://www.glassdoor.com/about/terms-2022-12-01/)). Por isso, a extensão local bloqueia LinkedIn, Jobbol e Glassdoor, incluindo seus subdomínios. Uma candidatura descoberta por esses serviços só poderá continuar em um portal externo do empregador/ATS, e apenas se esse destino permitir o preenchimento assistido ou fornecer integração autorizada.

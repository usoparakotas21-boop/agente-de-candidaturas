...
def make_manuscript() -> str:
    ...
    parts = [
        ...
        between(extra, "# CARDS DE BOLSO", "# PÁGINA FINAL"),
        between(extra, "# PÁGINA FINAL", "# ✅ Resumo das Adições"),
    ]
    return "\n\n---\n\n".join(parts)
...
def set_run_font(run, size=None, bold=None, italic=None, color=None, name="Aptos") -> None:
    run.font.name = name
    ...
    while i < len(lines):
        ...
        if numbered:
            if active_num_id is None:
                active_num_id = create_numbering_instance(doc, int(numbered.group(1)))

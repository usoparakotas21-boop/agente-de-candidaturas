...
OUT = ROOT / "tmp" / "disc_ebook" / "Cards_de_Bolso_DISC_Montserrat.pdf"
...
pdfmetrics.registerFont(TTFont("Body", str(FONT_DIR / "Inter-Regular.ttf")))
pdfmetrics.registerFont(TTFont("BodyBold", str(FONT_DIR / "Inter-Bold.ttf")))
pdfmetrics.registerFont(TTFont("BodyItalic", str(FONT_DIR / "Inter-Italic.ttf")))
pdfmetrics.registerFont(TTFont("Display", str(FONT_DIR / "Montserrat-ExtraBold.ttf")))
pdfmetrics.registerFont(TTFont("DisplayBold", str(FONT_DIR / "Montserrat-Bold.ttf")))
...
def cover(c):
    ...
    c.drawString(98, H - 242, "CARDS DE")
    c.drawString(98, H - 300, "BOLSO")
    x_disc = 98 + pdfmetrics.stringWidth("BOLSO ", "Display", 46)
    c.setFillColor(GREEN)
    c.drawString(x_disc, H - 300, "DISC")
    c.setStrokeColor(GREEN)
    c.setLineWidth(3)
    c.line(100, H - 333, W - 64, H - 333)
    c.setFont("DisplayBold", 21)
    c.drawString(100, H - 379, "Consulte 10 minutos antes")
    c.drawString(100, H - 408, "de abrir qualquer teste")
    c.setFillColor(TEXT)
    c.setFont("Body", 14)
    wrapped(c, "Os 4 perfis completos. As 4 combinações que mais aparecem em vagas reais.", 100, H - 470, W - 168, "Body", 14, TEXT, 20)
    wrapped(c, "As frases-âncora. O que pontuar no extremo. O que nunca pontuar.", 100, H - 535, W - 168, "BodyBold", 14, TEXT, 20)
    c.setFillColor(MUTED)
    c.setFont("BodyItalic", 11.5)
...
def back_cover(c):
    ...
    wrapped(c, "A empresa abriu o teste já sabendo o perfil que queria. Você abriu o teste já sabendo o código.", 56, H - 370, W - 112, "Body", 12.5, TEXT, 18)
    wrapped(c, "Da próxima vez, você vai ler a vaga como um briefing, mapear o perfil e manter a consistência até a assinatura do contrato.", 56, H - 422, W - 112, "Body", 12.5, TEXT, 18)
    c.setFillColor(PANEL)
    c.roundRect(52, 155, W - 104, 190, 12, fill=1, stroke=0)
    c.setFillColor(MUTED)
    c.setFont("BodyBold", 10)
    c.drawString(72, 310, "O LIVRO CONTOU")
    c.setFillColor(GREEN)
    c.setFont("Display", 30)
    c.drawString(72, 267, "DISC HACKED")
    wrapped(c, "Agora vai lá e responde o teste certo.", 72, 231, W - 144, "BodyBold", 12.5, TEXT, 17)
    c.setFillColor(GREEN)
    c.roundRect(72, 174, 250, 34, 8, fill=1, stroke=0)
    c.setFillColor(BG)
    c.setFont("BodyBold", 11)
    c.drawCentredString(197, 186, "CONTE SUA HISTÓRIA DE APROVAÇÃO")
    c.setFillColor(MUTED)

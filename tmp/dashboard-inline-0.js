      ...
      try {
        const response = await fetch("/auth/signup", {
          ...
          body: JSON.stringify({
            ...
            terms_accepted: $("signupTerms").checked,
            privacy_accepted: $("signupTerms").checked,
          }),
        });
    ...
    const healthLabels = {SAUDAVEL: "Risco baixo", ACEITAVEL: "Risco baixo", DUVIDOSA: "Revisar risco", SUSPEITA: "Risco alto"};
    ...
    const healthClass = band => ({SAUDAVEL:"safe", ACEITAVEL:"safe", DUVIDOSA:"review", SUSPEITA:"risk"}[band] || "review");
    ...
    const GUIDED_ONBOARDING_DISMISSED = "ac_guided_onboarding_v1";
    let guidedOnboardingStep = 0;
    let guidedOnboardingTrigger = null;

    function guidedOnboardingDismissed() {
      try { return window.localStorage.getItem(GUIDED_ONBOARDING_DISMISSED) === "1"; } catch { return false; }
    }

    function dismissGuidedOnboarding() {
      try { window.localStorage.setItem(GUIDED_ONBOARDING_DISMISSED, "1"); } catch { /* armazenamento local indisponível */ }
      const dialog = $("guidedOnboardingDialog");
      if (dialog?.open) dialog.close();
      if (guidedOnboardingTrigger?.focus) guidedOnboardingTrigger.focus();
    }

    function renderGuidedOnboardingStep() {
      document.querySelectorAll("[data-guided-step]").forEach(step => {
        step.hidden = Number(step.dataset.guidedStep) !== guidedOnboardingStep;
      });
      $("guidedBackButton").disabled = guidedOnboardingStep === 0;
      $("guidedNextButton").textContent = guidedOnboardingStep === 2 ? "Concluir" : "Próximo";
      $("guidedProgressBar").dataset.step = String(guidedOnboardingStep + 1);
    }

    function maybeShowGuidedOnboarding(hasProfile, hasPreferences) {
      if ((hasProfile && hasPreferences) || guidedOnboardingDismissed()) return;
      const dialog = $("guidedOnboardingDialog");
      if (!dialog || dialog.open) return;
      $("guidedProfileState").textContent = hasProfile ? "✓ Perfil já configurado" : "Ainda não configurado";
      $("guidedPreferencesState").textContent = hasPreferences ? "✓ Preferências já configuradas" : "Ainda não configuradas";
      guidedOnboardingStep = 0;
      renderGuidedOnboardingStep();
      window.setTimeout(() => {
        if (!dialog.open) {
          guidedOnboardingTrigger = $("onboardingNote")?.querySelector("a") || $("captureJobButton");
          dialog.showModal();
          $("guidedNextButton").focus();
        }
      }, 450);
    }

    async function syncOnboardingCard() {
      ...
      try {
        ...
        if (!profileResponse.ok || !preferencesResponse.ok) return;
        const profile = await profileResponse.json();
        const preferences = await preferencesResponse.json();
        const hasProfile = Boolean(profile.configured && (profile.name || profile.resume_filename));
        const hasPreferences = Boolean(
          (preferences.target_roles || []).length ||
          (preferences.locations || []).length ||
          (preferences.modalities || []).length
        );
        maybeShowGuidedOnboarding(hasProfile, hasPreferences);
        if (hasProfile && hasPreferences) {
          note.hidden = true;
        } else if (hasProfile) {
          note.querySelector("strong").textContent = "Defina suas preferências";
          note.querySelector("span").textContent = "Ajuste cargos, locais e modalidades para receber matches mais relevantes.";
          note.querySelector("a").textContent = "Abrir preferências →";
          note.querySelector("a").href = "/configuracoes#preferencias";
        }
      } catch {
        // O card estático continua como fallback se uma consulta falhar.
      }
    }
    ...
    function renderQueue(summary) {
      ...
      $("queueRows").innerHTML = visible.map(item => {
        ...
        const health = item.health_band ? `<span class="queue-health ${healthClass(item.health_band)}" title="${escapeHtml((item.health_signals || []).map(signal => signal.label).slice(0, 3).join("; "))}">● ${healthLabels[item.health_band] || "Risco a revisar"}${item.health_score == null ? "" : ` · ${item.health_score}/100`}</span>` : "";
        ...
        return `
          <tr>
            ...
            <td>${titleMarkup}<div class="company">${escapeHtml(item.company || "Empresa não identificada")}</div><div class="queue-reasons">${escapeHtml(reasons || "Sem motivos registrados")}</div>${health}</td>
    ...
    function renderDistribution() {
      ...
      $("distribution").innerHTML = active.length ? active.map(item => `
        ...
        <div class="bar"><span class="bar-fill-${Math.round(((item.count / max) * 100) / 10) * 10}"></span></div></div>`).join("") : '<div class="empty">Sem dados para exibir.</div>';
    }
    ...
    function renderTable() {
      ...
      document.querySelectorAll("#applicationRows tr").forEach(row => {
        row.addEventListener("click", () => openApplication(Number(row.dataset.id), row));
        row.addEventListener("keydown", event => { if (event.key === "Enter") openApplication(Number(row.dataset.id), row); });
      });
    }

    function openApplication(id, opener = null) {
      ...
      $("statusNote").value = "";
      const latestTracking = [...item.events].reverse().find(event => event.channel || event.external_result) || {};
      $("statusChannel").value = latestTracking.channel || "";
      $("externalResult").value = latestTracking.external_result || "";
      $("timeline").innerHTML = [...item.events].reverse().map(event => `
        ...
        <time>${formatDate(event.created_at)}</time>${event.note ? `<p>${escapeHtml(event.note)}</p>` : ""}${event.channel || event.external_result ? `<p class="muted">${event.channel ? `Canal: ${escapeHtml(event.channel)}` : ""}${event.channel && event.external_result ? " · " : ""}${event.external_result ? `Retorno: ${escapeHtml(event.external_result.replaceAll("_", " ").toLowerCase())}` : ""}</p>` : ""}</div>`).join("");
    ...
    function renderApplicationRisk(item) {
      const check = $("applicationRiskCheck");
      const acknowledgement = $("applicationRiskAcknowledgement");
      const acknowledgementInput = $("riskAcknowledged");
      const signals = item.health_signals || [];
      const isReview = item.health_band === "DUVIDOSA";
      const isRisk = item.health_band === "SUSPEITA" || item.fraud_suspected;
      check.hidden = !item.health_band;
      acknowledgement.hidden = !isReview;
      acknowledgementInput.checked = Boolean(item.risk_reviewed_at);
      acknowledgementInput.disabled = !isReview || Boolean(item.risk_reviewed_at);
      $("applicationRiskSignals").innerHTML = signals.length
        ? signals.slice(0, 4).map(signal => `<li>${escapeHtml(signal.label || signal.code || "Sinal identificado")}</li>`).join("")
        : "";
      if (!item.health_band) return;
      if (isRisk) {
        $("applicationRiskTitle").textContent = "Risco alto detectado";
        $("applicationRiskMessage").textContent = "Não avance enquanto a origem, a empresa e qualquer pedido de pagamento não forem verificados por canais oficiais.";
      } else if (isReview) {
        $("applicationRiskTitle").textContent = "Revise a segurança da vaga";
        $("applicationRiskMessage").textContent = "Encontramos sinais que merecem conferência. Verifique empresa, domínio e condições antes de abrir o anúncio.";
      } else {
        $("applicationRiskTitle").textContent = "Verificação de segurança concluída";
        $("applicationRiskMessage").textContent = "Não identificamos sinais críticos nesta captura. Continue conferindo a vaga no site oficial.";
      }
      $("startApplicationButton").disabled = isRisk || (isReview && !item.risk_reviewed_at);
    }
    ...
    function setApplicationDrawerLoading(loading) {
      ...
      ["drawerTitle", "drawerCompany", "drawerScore", "drawerPersonalization", "drawerDecision", "drawerRecommendation"].forEach(id => {
        $(id).classList.toggle("skeleton", loading);
      });
      ...
      $("statusChannel").disabled = loading;
      $("externalResult").disabled = loading;
    ...
    function showApplicationLoading() {
      ...
      $("applicationRiskCheck").hidden = true;
      $("markApplicationSentButton").hidden = true;
      $("markApplicationSentButton").disabled = true;
      ...
      $("interviewPrepButton").hidden = true;
      $("interviewPrepCard").hidden = true;
    ...
    function renderAnalysis(applicationId) {
      ...
      $("interviewPrepButton").hidden = !analysis;
      if (!analysis) return;
    ...
    async function loadInterviewPrep() {
      ...
      const button = $("interviewPrepButton");
      button.disabled = true;
      try {
        const response = await fetch(`/api/interviews/prep/${selectedId}`);
        if (!response.ok) throw new Error("Não foi possível montar a preparação agora.");
        const prep = await response.json();
        $("interviewPrepIntro").textContent = `${prep.job_title || "Esta vaga"}: revise as perguntas abaixo e responda com fatos comprovados.`;
        $("interviewQuestionList").innerHTML = (prep.questions || []).map(question => `<li>${escapeHtml(question)}</li>`).join("");
        $("interviewAnswerFramework").textContent = prep.answer_framework || "Use contexto, ação e resultado.";
        $("interviewPrepCard").hidden = false;
      } catch (error) {
        toast(error.message);
      } finally {
        button.disabled = false;
    ...
    async function saveStatus() {
      ...
      try {
        const response = await fetch(`/applications/${selectedId}/status`, {
          ...
          body: JSON.stringify({status: $("newStatus").value, note: $("statusNote").value.trim(), channel: $("statusChannel").value, external_result: $("externalResult").value})
        });
...
async function downloadCoverLetterDocument() {
  ...
  try {
    ...
    async function startDocumentExportCheckout() {
      ...
      try {
        const response = await fetch("/billing/document-export/checkout", {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({application_id: selectedId})});
        const payload = await response.json();
    ...
    $("closeGuidedOnboarding").addEventListener("click", dismissGuidedOnboarding);
    $("skipGuidedOnboarding").addEventListener("click", dismissGuidedOnboarding);
    $("guidedBackButton").addEventListener("click", () => {
      guidedOnboardingStep = Math.max(0, guidedOnboardingStep - 1);
      renderGuidedOnboardingStep();
    });
    $("guidedNextButton").addEventListener("click", () => {
      if (guidedOnboardingStep >= 2) {
    ...
    $("guidedCaptureButton").addEventListener("click", () => {
      dismissGuidedOnboarding();
      guidedOnboardingTrigger = $("captureJobButton");
      resetCaptureReview();
      $("captureJobDialog").showModal();
    });
    document.querySelectorAll("[data-guided-link]").forEach(link => link.addEventListener("click", dismissGuidedOnboarding));
    ...
    $("riskAcknowledged").addEventListener("change", async event => {
      if (!event.target.checked || !selectedId) return;
      event.target.disabled = true;
      try {
        const response = await fetch(`/applications/${selectedId}/risk-review`, {method: "POST"});
        if (!response.ok) throw new Error(await responseError(response, "Não foi possível registrar a revisão de risco."));
        const updated = await response.json();
        applications = applications.map(item => item.id === updated.id ? updated : item);
        openApplication(updated.id);
        toast("Revisão de risco registrada. Confira o anúncio no site oficial.");
      } catch (error) {
    ...
    $("interviewPrepButton").addEventListener("click", loadInterviewPrep);

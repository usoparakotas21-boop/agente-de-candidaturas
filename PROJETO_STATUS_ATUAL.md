...
- Na validação ampla de 21/09, **353 testes Python passaram**. O módulo `test_ats_registry_domains.py` ficou fora da execução filtrada porque o runtime disponível não tem `pytest`; sua execução anterior separada registrou 29/29. A checagem `git diff --check` passou.

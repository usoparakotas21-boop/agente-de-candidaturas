...
| P1.19 | E-mails de ciclo de vida | **71% (estimativa)** | Follow-up após 7 dias e avisos de entrevista têm opt-in, outbox privada, retries, agrupamento diário/semanal e retenção de 60 dias. A captação individual passou a preservar e exibir o `JobPosting.validThrough` estruturado quando o anúncio o fornece; vagas sem data explícita continuam sem prazo. Falta implementar e testar o dispatcher dos alertas de expiração, além de inserir `SMTP_PASSWORD` no Render e validar envio real em caixa confirmada. |
...
| P2.7+ | Domínio próprio, Kanban, exportação e extensão de navegador | **95% configuração do domínio (estimativa)** | Nameservers e DNS do domínio raiz/`www` resolvem, o site está acessível por HTTPS e `APP_BASE_URL` foi aplicado no Blueprint e está Live. Falta revisar os redirects/callbacks de Supabase, Google, Microsoft e Mercado Pago. O complemento Chrome 0.1.0 está implementado e empacotado; Kanban e exportação continuam no escopo de escala. |

### Atualização operacional — 21/09/2026

- **P1.22 — preenchimento assistido no Chrome:** há agora um complemento MV3 com permissão temporária `activeTab`, consentimento a cada página, importação local do perfil e preenchimento dos campos de texto reconhecidos que estejam vazios. Ignora dados já presentes, senhas, uploads, CAPTCHAs, consentimentos, perguntas abertas e valores que não cabem no limite do portal. Não faz rede, login ou envio. `node --test tests/chrome-extension.test.cjs` passou **5/5**, os três arquivos JavaScript passaram `node --check`, o manifesto JSON foi validado e o ZIP instalável foi gerado. A submissão automática permanece desligada até um endpoint oficial e autorização documentada por ATS/empregador; nos portais cujas regras não permitem preenchimento assistido, continua o processo manual.
- **P1.19 — datas de validade de vagas:** o importador da vaga individual agora lê somente o `validThrough` explícito do JSON-LD `JobPosting`, normaliza fuso/data sem inferir prazo a partir do texto e armazena/exibe a data quando disponível. Testes cobrem prazo com data válida e dado ausente/inválido; as vagas antigas continuam sem prazo. Isto prepara a fonte de dados, mas não ativa e-mails de expiração: falta o dispatcher, `SMTP_PASSWORD` e prova de entrega real.

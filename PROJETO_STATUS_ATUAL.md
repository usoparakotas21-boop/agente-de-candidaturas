...
**Commit de aplicação mais recente:** `7628cca` — `Add profile load retry on resumes page` (publicado no Render; deploy `b80203e` Live)
...
| MFA | Enrollment/status/unenroll e challenge/verify TOTP; deploy `03c0adb` Live exige AAL2 em rotas protegidas, sessão renovada e callback, e conserva a sessão elevada depois do verify; 202 testes passam | **P0.3** aguarda um login real com TOTP; recuperação automática após perda dos dispositivos ainda não existe e está informada na tela |

...
**E-mail de contato:** o endereço foi publicado na interface; Cloudflare contém MX do Zoho (`mx.zoho.com`, `mx2.zoho.com`, `mx3.zoho.com`) e o resolvedor público 1.1.1.1 confirma os três registros. O SPF publicado autoriza o relay Brevo (`v=spf1 include:spf.brevo.com ~all`). No Render, a variável `SMTP_PASSWORD` agora aparece salva junto com `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME`, `SMTP_PORT=587` e `SMTP_USE_TLS=true`; o valor segue mascarado. A chave SMTP ativa na Brevo teve uso em 21/09/2026. Ainda falta comprovar o envio real de recibo, documentos e notificações; o Render Shell não está disponível no plano Free e a sessão do aplicativo está desconectada nesta conferência.
...
| P1.10 | Geração paga, recibo e entrega por e-mail | **92%** | O domínio está autenticado no Brevo e o relay está pronto. `SMTP_PASSWORD` agora aparece na lista de variáveis salvas no Render; a Brevo mostra uma chave SMTP ativa e informa uso em 21/09/2026. Ainda falta comprovar autenticação/entrega real para recibo + currículo + carta em uma caixa confirmada. O Render Shell está bloqueado no plano Free, então a verificação deve ocorrer por um fluxo autenticado do aplicativo. |
...
- **P1.10/P1.19/P1.18 — SMTP:** revalidei o Render sem revelar valores; `SMTP_PASSWORD` aparece na lista de variáveis salvas e a Brevo mostra uma chave ativa mascarada, com último uso em 21/09/2026. Não substituí nem revelei a chave. O Render Shell informa que não é suportado no plano Free; a sessão atual do site exige login, então ainda não consegui executar o teste de envio por um fluxo autenticado. Nenhum segredo foi copiado para o chat. O próximo teste é repetir o envio pelo histórico pago após o proprietário entrar no aplicativo e confirmar que o recibo/documentos chegam à caixa correta.

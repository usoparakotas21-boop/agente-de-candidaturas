...
**E-mail de contato:** o endereço foi publicado na interface; os registros DNS salvos mostram MX nulo (`.`) e SPF `-all`, sem recebimento configurado. O Brevo mostra o domínio autenticado, relay SMTP disponível e uma chave ativa mascarada. No Render, `SMTP_PORT` e `SMTP_USE_TLS` existem, mas `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME` e `SMTP_PASSWORD` estão ausentes. O editor do Cloudflare está aberto com uma proposta de SPF ainda não salva; ela foi preservada. Não considerar recebimento nem envio operacional até concluir DNS, instalar as credenciais no Render e testar.
...
| P1.10 | Geração paga, recibo e entrega por e-mail | **92%** | O domínio está autenticado no Brevo e o relay SMTP está disponível; `SMTP_PORT` e `SMTP_USE_TLS` existem no Render. Faltam `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME` e `SMTP_PASSWORD`, além da prova de recibo + currículo + carta em uma caixa confirmada. |
...
| P1.19 | E-mails de ciclo de vida | **70% (estimativa)** | Follow-up após 7 dias e avisos de entrevista têm opt-in, outbox privada, retries, agrupamento diário/semanal e retenção de 60 dias. Faltam as quatro variáveis SMTP de conexão/remetente e validar envio real em caixa confirmada; aviso de expiração fica indisponível até haver uma data de vencimento confiável. |

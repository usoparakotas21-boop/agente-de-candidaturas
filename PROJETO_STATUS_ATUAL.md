...
- A verificação contra senhas comprometidas usa k-anonimato (somente prefixo de 5 caracteres do SHA-1, nunca a senha ou o hash completo) e `PWNED_PASSWORD_CHECK` está presente no Render. O fluxo MFA de login, desafio, revogação e expiração já está implementado; a tela de segurança em produção foi validada. O Security Advisor do Supabase ainda aponta `Leaked Password Protection Disabled`, portanto a proteção nativa precisa ser ativada no painel antes de encerrar o P0.10.
...
- A suíte automatizada em `tests/` foi reexecutada após o endurecimento do CSP e do vínculo transacional dos exports: **121 testes aprovados em 10,07 s**, com 6 avisos de depreciação sem falhas. Os scripts legados na raiz continuam fora da suíte porque dependem de servidores locais em 8001/8002.
...
| Senhas comprometidas | Consulta k-anonimizada envia apenas o prefixo do hash SHA-1 para o serviço de verificação; `PWNED_PASSWORD_CHECK` aparece no Render sem expor valor | Código e Render confirmados; Security Advisor do Supabase ainda acusa proteção nativa desativada — pendência **P0.10** |

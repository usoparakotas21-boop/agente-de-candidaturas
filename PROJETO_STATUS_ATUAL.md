...
**E-mail de contato:** o endereço foi publicado na interface; Cloudflare contém MX do Zoho (`mx.zoho.com`, `mx2.zoho.com`, `mx3.zoho.com`) e o resolvedor público 1.1.1.1 confirma os três registros. O SPF publicado autoriza o relay Brevo (`v=spf1 include:spf.brevo.com ~all`). No Render, a variável `SMTP_PASSWORD` aparece salva junto com `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME`, `SMTP_PORT=587` e `SMTP_USE_TLS=true`; o valor segue mascarado. A chave SMTP ativa na Brevo teve uso em 21/09/2026. A sessão autenticada atual mostra documentos pagos na biblioteca, mas o e-mail está marcado como não enviado e pede confirmação do endereço da conta. Após confirmar, é possível usar o botão de reenvio. O Render Shell não está disponível no plano Free.
...
- **P1.10/P1.22 — reenvio de documentos:** o commit `58f973b` corrigiu o `KeyError: 'document-email-retry'` que dava HTTP 500, assim como os limites ausentes para prévia/envio de candidatura por e-mail. O Render confirmou `58f973b` Live; o botão retornou HTTP 200 e agora a rota alcança SMTP, mas a biblioteca mostra `Falha no envio`/`Falha de transporte SMTP; tente novamente`. O log confirma falha no envio, sem expor o motivo. Na Brevo, a chave SMTP atual está ativa, porém seu campo de última utilização está vazio. Adicionei telemetria sanitizada que registra somente fase, classe da exceção e código SMTP numérico opcional. **31 testes focados e 411 testes `unittest` passaram**; o módulo pytest-only `test_ats_registry_domains.py` foi excluído porque o runtime do projeto nega a inicialização e o runtime alternativo não tem a extensão nativa necessária para `pytest`. A nova telemetria aguarda deploy; nenhum e-mail foi confirmado nesta tentativa.

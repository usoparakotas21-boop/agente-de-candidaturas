...
3. **MFA opcional por conta — proteção ponta a ponta implementada localmente:** quem tem TOTP confirmado precisa de `aal2` em toda rota protegida; sessões `aal1` antigas, renovadas ou obtidas no callback de confirmação são bloqueadas e voltam ao login com desafio, enquanto contas sem fator continuam sem MFA obrigatório. A verificação de enrollment preserva a sessão elevada devolvida pelo Supabase. Cookies são limpos com `HttpOnly`, `Secure` e `SameSite=Lax`; desafios expirados ou respostas sem `aal2` não criam sessão. A tela deixa explícito que SMS/códigos de recuperação não existem e orienta a cadastrar o mesmo segredo TOTP em dois dispositivos antes da ativação. A suíte oficial passou em **202 testes**. **P0.3 ainda aguarda deploy e login real com TOTP**; não há recuperação automática se os dois dispositivos autenticadores forem perdidos.
...
| MFA | Enrollment/status/unenroll e challenge/verify TOTP; código local exige AAL2 em rotas protegidas, sessão renovada e callback, e conserva a sessão elevada depois do verify; 202 testes passam | Publicar a correção e confirmar um login real com TOTP em **P0.3**; recuperação automática após perda dos dispositivos ainda não existe e está informada na tela |

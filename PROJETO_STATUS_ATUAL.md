...
- **P1.22 — Copiloto Chrome 1.0.0 (`dd483b5`, Render Live):** pacote estável reconstruído com 17 arquivos, incluindo ícones de marca. Listagem e nota de permissões/privacidade foram preparadas; ZIP validado (CRC íntegro), instalação manual documentada e a versão publicada na página Perfil foi atualizada. `15/15` testes Node da extensão, `32/32` testes de shell, sintaxe JS e `git diff --check` passaram. A distribuição estável está no ar pelo ZIP; ainda não foi enviada/publicada na Chrome Web Store.
...
- **P1.22 — Copiloto: matriz de portais (preparada):** a próxima versão adiciona um dicionário modular de seletores profissionais para Gupy, Vagas.com, InfoJobs, Catho, Sólides e Empregos.com.br, com setter nativo e eventos para campos React e fallback por rótulos. Campos sensíveis, perguntas abertas, consentimento, uploads e envio final continuam fora do escopo; listas suspensas personalizadas ficam para revisão manual. LinkedIn, Glassdoor e Jobbol continuam bloqueados pela política de automação. `16/16` testes Node, sintaxe JavaScript e `git diff --check` passaram; o ZIP será reconstruído e publicado após o commit.

...
| P1.22 — prioridade máxima do produto | Candidatura automática por canal autorizado | **15% (estimativa)** | Perfil, análise, currículo/carta direcionados e histórico já existem. O fluxo de Vagas prepara os documentos em um clique (`87f542f` Live). **Nenhuma candidatura é enviada automaticamente hoje:** para iniciar o primeiro conector faltam a API/convênio e autorização específica de um empregador/ATS, além do consentimento explícito da pessoa para cada vaga/destino. A coleta de vagas, por si só, não autoriza envio. A governança exige `automated_submission`; não usar extensão/bot para contornar termos, login, CAPTCHA ou controles. |
...
| P2.7+ | Domínio próprio, Kanban, exportação e extensão de navegador | **95% configuração do domínio (estimativa)** | Nameservers e DNS do domínio raiz/`www` resolvem, o site está acessível por HTTPS e `APP_BASE_URL` foi aplicado no Blueprint e está Live. Falta revisar os redirects/callbacks de Supabase, Google, Microsoft e Mercado Pago. Kanban, exportação e extensão continuam no escopo de escala. |

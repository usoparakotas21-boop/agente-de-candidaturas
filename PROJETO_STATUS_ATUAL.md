...
**Domínio próprio:** `candidaturacerta.com.br` está criado na Cloudflare, mas a zona ainda aparece como pendente; faltam registros DNS para o site e associação do domínio no Render. O endereço Render permanece como base até a validação.
...
| P1 | Consultoria por R$ 197 (pagamento único) | **0% contratação** | Preço e entregáveis estão definidos na landing, mas a contratação está corretamente desativada. Falta checkout avulso e processo operacional de agendamento/entrega, incluindo canal de contato real. |
...
| P2.7+ | Domínio próprio, Kanban, exportação e extensão de navegador | **0% integração do domínio** | Marca e identidade visual Candidatura Certa estão Live; zona Cloudflare ainda pendente, sem registros raiz/www do site, e domínio ainda não associado no Render. Preservar primeiro a configuração de e-mail. Kanban, exportação e extensão continuam no escopo de escala. |
...
- A zona `candidaturacerta.com.br` na Cloudflare ainda está **Pending**. Na leitura dos registros há apenas um MX nulo (`.`), SPF `-all` e DMARC `p=reject`; não há A/CNAME de raiz ou `www`, nem provedor de e-mail configurado. Por isso, o domínio e o recebimento no e-mail de contato não foram declarados prontos. Configurar e validar e-mail antes de substituir nameservers/registros; depois associar o domínio no Render e verificar HTTPS/redirect.
...
| Cloudflare, DNS redundante e DDoS | Domínio próprio confirmado: `candidaturacerta.com.br`; zona criada mas pendente, sem DNS do site e sem provedor de e-mail | **P2.7**; manter Render operacional, concluir DNS de e-mail/site com validação, associar domínio ao Render e validar HTTPS/redirect; só então avaliar Cloudflare como borda |
...
7. **P2.7 — Configurar `candidaturacerta.com.br`:** domínio do proprietário confirmado e marca publicada; falta concluir a transição de nameservers, configurar DNS de e-mail e do site sem interromper recebimento, associar o custom domain no Render e validar HTTPS/redirect e monitoramento. A zona Cloudflare está pendente e não tem registros raiz/www.
...
- O domínio próprio `candidaturacerta.com.br` foi confirmado pelo proprietário; a configuração no Render/DNS permanece em **P2.7**. A zona Cloudflare está pendente; a produção continua no hostname Render. O e-mail de contato foi publicado, mas aguarda provedor/MX e teste real.

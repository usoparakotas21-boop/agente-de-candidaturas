...
| P1.22 — prioridade máxima do produto | Candidatura automática por canal autorizado | **40% (estimativa)** | Perfil, análise, documentos direcionados, histórico e Copiloto Chrome 1.0.0 autenticado, com botão persistente opcional por domínio, preparação, análise contextual da vaga, consentimento explicado em linguagem simples, sugestões Gemini com opt-in, cópia rápida e preenchimento consentido estão implementados e publicados. A versão estável 1.0.0 está Live no Render e distribuída por ZIP para instalação manual; a Chrome Web Store ainda não publicou o pacote. **Submissão automática continua em 0%:** não há conector oficial autorizado. Faltam API/convênio e autorização do empregador/ATS, além de consentimento por vaga/destino. A tentativa de reenvio de documento em produção encontrou `KeyError` por limites de taxa ausentes; o mesmo defeito afetava as rotas de prévia/envio por e-mail a recrutadores. O código corrigido aguarda deploy e reteste. O complemento não envia formulários nem contorna CAPTCHAs ou proteções; para cada portal, o autofill só pode ser usado quando permitido. |
| P1.10 | Geração paga, recibo e entrega por e-mail | **92%** | O domínio está autenticado no Brevo e o relay está pronto. `SMTP_PASSWORD` aparece na lista de variáveis salvas no Render; a Brevo mostra uma chave SMTP ativa e informa uso em 21/09/2026. Há documentos pagos na biblioteca. O primeiro reenvio em produção falhou antes de contactar SMTP: os logs do Render mostraram `KeyError: 'document-email-retry'` na rota, não uma falha de credencial. O código agora define limites para reenvio de documentos, prévia e envio de candidatura; os testes locais passaram. Falta o deploy do conserto e uma entrega real confirmada de recibo/documentos. O Render Shell não está disponível no plano Free. |
...
- **P1.10/P1.22 — falha real de reenvio corrigida no código, aguardando deploy:** o botão de reenvio da biblioteca retornou erro genérico. Os logs do Render identificaram HTTP 500 em `retry_document_delivery`, causado por `KeyError: 'document-email-retry'`, antes de qualquer conexão SMTP. Uma auditoria de todas as chamadas encontrou o mesmo problema potencial nos limites `application-email-preview` e `application-email-send`. Foram adicionados orçamentos por IP/conta para os três escopos (3 reenvios/15 min, 30 prévias/h e 5 envios/15 min) e testes contra regressão. **17 testes direcionados e 411 testes `unittest` passaram**; a suíte `unittest` foi executada excluindo o módulo pytest-only `test_ats_registry_domains.py`, indisponível nesse runtime. Ainda não houve envio SMTP real nesta tentativa. O deploy documental `e417d07` está em andamento; o commit com o conserto será publicado separadamente e só então o botão será testado novamente.

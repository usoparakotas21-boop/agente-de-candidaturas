...
| P1.22 — prioridade máxima do produto | Candidatura automática por canal autorizado | **40% (estimativa)** | Perfil, análise, documentos direcionados, histórico e Copiloto Chrome 1.0.0 autenticado, com botão persistente opcional por domínio, preparação, análise contextual da vaga, consentimento explicado em linguagem simples, sugestões Gemini com opt-in, cópia rápida e preenchimento consentido estão implementados e publicados. A versão estável 1.0.0 está Live no Render e distribuída por ZIP para instalação manual; a Chrome Web Store ainda não publicou o pacote. **Submissão automática continua em 0%:** não há conector oficial autorizado. Faltam API/convênio e autorização do empregador/ATS, além de consentimento por vaga/destino. O complemento não envia formulários nem contorna CAPTCHAs ou proteções; para cada portal, o autofill só pode ser usado quando permitido. |
| P1.10 | Geração paga, recibo e entrega por e-mail | **92%** | O domínio está autenticado no Brevo e o relay está pronto. `SMTP_PASSWORD` aparece na lista de variáveis salvas no Render; a Brevo mostra uma chave SMTP ativa e informa uso em 21/09/2026. Na conta autenticada atual há documentos pagos na biblioteca, mas o estado de e-mail pede confirmação do endereço antes do envio. Falta confirmar esse endereço e acionar o reenvio para comprovar recibo/documentos em uma caixa confirmada. O Render Shell não está disponível no plano Free. |

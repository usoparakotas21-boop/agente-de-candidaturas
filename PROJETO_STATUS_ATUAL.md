...
**E-mail de contato:** o endereço foi publicado na interface; Cloudflare contém MX do Zoho (`mx.zoho.com`, `mx2.zoho.com`, `mx3.zoho.com`) e o resolvedor público 1.1.1.1 confirma os três registros. O SPF publicado autoriza o relay Brevo (`v=spf1 include:spf.brevo.com ~all`). No Render, `SMTP_PASSWORD` aparece salva com os demais campos do relay; o valor segue mascarado. Após o primeiro teste controlado expirar na fase de conexão usando STARTTLS, a configuração operacional foi ajustada para `SMTP_PORT=465` e `SMTP_USE_TLS=false` (deploy de ambiente `dep-daoqc6v40ujc73898oug` Live), e o código passou a selecionar `SMTP_SSL` automaticamente nessa porta. A chave SMTP ativa na Brevo teve uso em 21/09/2026. A sessão autenticada mostra documento pago na biblioteca; a nova tentativa ainda precisa confirmar a entrega na caixa. O Render Shell não está disponível no plano Free.
...
- **P1.10/P1.22 — reenvio de documentos:** o commit `58f973b` corrigiu o `KeyError: 'document-email-retry'` que dava HTTP 500, assim como os limites ausentes para prévia/envio de candidatura por e-mail. O Render confirmou `58f973b` Live; o botão retornou HTTP 200, mas a primeira tentativa expirou em `smtp_stage=connect` antes da autenticação. O log sanitizado registra somente fase, classe da exceção e código SMTP numérico opcional. O ambiente foi ajustado para relay com TLS implícito (`SMTP_PORT=465`, `SMTP_USE_TLS=false`) e o código agora seleciona `SMTP_SSL` nessa configuração; o novo commit aguarda publicação. **31 testes focados e 411 testes `unittest` passaram**; o módulo pytest-only `test_ats_registry_domains.py` foi excluído porque o runtime do projeto nega a inicialização e o runtime alternativo não tem a extensão nativa necessária para `pytest`. Nenhum e-mail foi confirmado nesta tentativa.

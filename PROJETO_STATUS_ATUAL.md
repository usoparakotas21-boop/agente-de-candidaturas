...
| P1.11 | Fechamento dos textos legais | **70% (estimativa)** | Páginas públicas e consentimento versionado existem; falta inserir os dados finais do controlador, contato, retenção e subprocessadores. |
...
7. **P2.7 — Configurar `candidaturacerta.com.br`:** domínio do proprietário confirmado; falta configurar o custom domain no Render, registros DNS, HTTPS/redirect, monitoramento e, se necessário, proxy da Cloudflare para filtrar tráfego L7 antes do Render.
...
- O domínio próprio `candidaturacerta.com.br` foi confirmado pelo proprietário; a configuração no Render/DNS permanece classificada como **P2.7** e ainda não foi aplicada.

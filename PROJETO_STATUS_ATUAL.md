...
| P1 | Consultoria recorrente | **0% contratação** | Modelo definido: R$ 197/mês, com 1 atendimento individual por mês. O checkout recorrente do Mercado Pago e o processo de agendamento/entrega ainda precisam ser implementados; a contratação permanece desativada. |
...
- O restante do P1 está corretamente separado entre bloqueios de integração/produção e decisões do proprietário: SMTP e testes de entrega (P1.10/P1.19), contratação recorrente (Start/Pro), dados legais (P1.11), canal de suporte (P1.18), revisão visual autenticada (P1.17), aprovação de fontes (P1.20) e operação/entrega editorial (e-book e consultoria). **P1.6 foi fechado com confirmação da exclusão em produção.**
...
- **Assinaturas mensais Mercado Pago:** a landing e o backend definem Start a R$ 34,90/mês e Pro a R$ 99,00/mês, com preapproval mensal, webhook e cancelamento; 9 testes locais cobrem os preços/payloads dos planos, webhook, cancelamento e proteção do e-book. O ciclo ainda não foi exercitado com uma assinatura real em produção.

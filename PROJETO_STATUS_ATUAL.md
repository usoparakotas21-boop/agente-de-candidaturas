...
**Git e produção:** a correção de `/candidaturas` está no commit `f9ca3ff`, sincronizado com o GitHub e confirmado como **Live** no Render em 21/09/2026, às 00:18 BRT; o serviço respondeu ao health check com HTTP 200.
...
| P1.19 | E-mails de ciclo de vida | **55%** | O follow-up opcional de 7 dias está publicado; falta SMTP e avaliar lembretes de entrevista/expiração com eventos e opt-in reais. E-mail de boas-vindas duplicaria a confirmação atual; aviso de mudança de etapa é redundante enquanto o próprio usuário altera os status. |
...
| P1.17 | Polimento visual residual | **95% (estimativa)** | A correção da lista de candidaturas, o skeleton e retry estão Live no Render. Falta a revisão visual final dos fluxos menos usados. |
...
- **P1.19 — primeira entrega publicada (`d385cd8`, Render Live):** lembrete opcional e único por candidatura após 7 dias sem retorno, com agrupamento por frequência, outbox persistente, retentativas e retenção de 60 dias. Só usa o e-mail confirmado da conta Supabase, nunca o contato importado do currículo. O consentimento agora é explícito e fica desligado por padrão, inclusive para preferências antigas. Os 12 testes do módulo passaram. Nesta rodada, 308 testes da suíte `unittest` passaram; o módulo ATS de 29 testes foi executado separadamente com o pytest empacotado. O envio real permanece desligado até configurar `SMTP_*` no Render; lembretes de entrevista/expiração aguardam SMTP e eventos adequados. E-mail de boas-vindas duplicaria a confirmação do Supabase e fica adiado; aviso de mudança de status não agrega enquanto a própria ação de atualizar a etapa é manual.
...
- **P1.17 — correção adicional publicada (`f9ca3ff`, Render Live):** `/candidaturas` carrega primeiro a lista principal; falhas nas métricas ou nos lembretes ficam isoladas, com estado acessível e retry próprio. A lista inicial/retry usa skeleton; três cenários simulados cobrem falha independente de métricas, falha de lembretes e indisponibilidade da lista. O health check do deploy respondeu HTTP 200.
...
| Erros de validação orientados à ação | `/curriculos`, entrevistas, lookup do estúdio e `/candidaturas` mantêm retry; falhas auxiliares de métricas/lembretes não ocultam mais a lista | **100% código nesta tela** | **P1.17**, falta confirmar a publicação e concluir a revisão residual |
...
| E-mails de Customer Success | Lembrete único após 7 dias sem retorno implementado com opt-in explícito, outbox idempotente, retentativas e exclusão/portabilidade LGPD; envio real aguarda configuração SMTP. Boas-vindas, entrevista, mudança de status e expiração ainda não existem; recibo/documentos pagos ficam em **P1.10** | **55%** | **P1.19**, sem duplicar o escopo de entrega paga |
...
17. **P1.17 — primeiro acesso e polimento:** o tour guiado de três passos salva o progresso por conta, retoma ao voltar de currículo/preferências e só dispensa por ação explícita. Entrevistas e a seleção/vínculo da vaga no estúdio têm skeleton, `aria-busy`, retry e estados vazios/erro; todos respeitam movimento reduzido. `/perfil` agora permite editar experiências, competências, formação e idiomas; só as seções alteradas ficam marcadas como manuais, e reimportações preservam esses dados, os valores não encontrados e os metadados do perfil. Corrigimos a lista para manter múltiplas linhas e o botão para aceitar salvamentos repetidos. Já estavam publicados hover dos botões, pulso do status conectado, feedback de upload/OCR, skeleton e retry no Banco de vagas/drawer e favicon `AC`. O commit `414576f` está **Live** no Render; a suíte passou em **281 testes** (sem o módulo que depende de `pytest`, ausente no runtime empacotado), `py_compile`, validação sintática do JavaScript e `git diff --check`.

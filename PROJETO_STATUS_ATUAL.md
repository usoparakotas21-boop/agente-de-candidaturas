...
| P1.21 | Simulador contextual e feedback de entrevista por IA | **90% (estimativa)** | Fluxo contextual, reinício seguro, validação/limites da resposta Gemini e fallback do roteiro para dados antigos ou malformados estão Live em `58d286a`, `b7bfe46` e `8c604fd`. `GEMINI_API_KEY` existe no Render, mas o valor ficou mascarado. A aba de produção atualmente mostra a tela de login; faltam uma sessão Pro autenticada e uma chamada real ao modelo para confirmar validade e fluxo ponta a ponta. |

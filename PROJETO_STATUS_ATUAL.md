...
**Git e produção:** a marca Candidatura Certa, o favicon, os contatos públicos, o chat de suporte e o Copiloto Chrome 0.6.0 estão publicados. O commit `8646f60` está **Live** no Render. Esta rodada confirmou o deploy no painel do Render; a checagem direta de `/health` e do ZIP foi bloqueada pelo ambiente do navegador/rede, portanto não atualizo a evidência HTTP do checkpoint anterior.
...
| P2.7+ | Domínio próprio, Kanban, exportação e extensão de navegador | **95% configuração do domínio (estimativa)** | Nameservers e DNS do domínio raiz/`www` resolvem, o site está publicado por HTTPS e `APP_BASE_URL` está no Render. Falta revisar os redirects/callbacks de Supabase, Google, Microsoft e Mercado Pago. O Copiloto Chrome 0.6.0 está Live, distribuído por ZIP para instalação manual; Chrome Web Store ainda não publicado. Kanban e exportação continuam no escopo de escala. |

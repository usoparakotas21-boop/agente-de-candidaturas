...
| P1.22 — prioridade máxima do produto | Candidatura automática por canal autorizado | **25% (estimativa)** | Perfil, análise, documentos direcionados, histórico e complemento Chrome 0.2.0 de preparação assistida já existem. **Envio automático continua em 0%:** não há conector oficial autorizado. Para submissão em nome do usuário faltam API/convênio e autorização de um empregador/ATS, além de consentimento por vaga/destino. O complemento não envia formulários; seu uso deve ser permitido pelo portal e nunca pode contornar login, CAPTCHA, limites ou controles. |
...
| P2.7+ | Domínio próprio, Kanban, exportação e extensão de navegador | **95% configuração do domínio (estimativa)** | Nameservers e DNS do domínio raiz/`www` resolvem, o site está acessível por HTTPS e `APP_BASE_URL` foi aplicado no Blueprint e está Live. Falta revisar os redirects/callbacks de Supabase, Google, Microsoft e Mercado Pago. O complemento Chrome 0.2.0 está implementado e empacotado; Kanban e exportação continuam no escopo de escala. |

### Atualização operacional — 21/09/2026

- **P1.22 — preparação de candidaturas no Chrome 0.2.0:** após a pessoa confirmar que o portal permite autofill e autorizar aquela página, o complemento MV3 preenche campos de texto reconhecidos e selects de cidade/estado apenas com opção exata do perfil. Usa `activeTab`, guarda o perfil apenas em `chrome.storage.local`, exige HTTPS, e não faz rede, login, upload, CAPTCHA ou envio. `node --test tests/chrome-extension.test.cjs` passou **6/6**; a submissão automática continua em **0%** até existir API/convênio oficial e autorização documentada por ATS/empregador. |
- **P1.22 — viabilidade dos conectores revisada em fontes oficiais:** LinkedIn exige aprovação de parceiro/certificação; Indeed exige integração de parceiro sob os termos da API; Lever precisa de uma chave emitida pela conta empregadora; e a API de criação de aplicação da Gupy exige token de recrutamento da empresa e, para candidatos inseridos manualmente, convite aceito pela própria pessoa. Nenhuma dessas credenciais é uma autorização geral vinculada à conta do candidato. Referências e escopo documentados em `SOURCE_GOVERNANCE.md`.

...
**E-mail de contato:** o endereço foi publicado na interface; os registros DNS salvos mostram MX nulo (`.`) e SPF `-all`, sem recebimento configurado. O Brevo mostra o domínio autenticado, relay SMTP disponível e uma chave ativa mascarada. No último estado salvo do Render, `SMTP_PORT` e `SMTP_USE_TLS` existem, mas `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME` e `SMTP_PASSWORD` estão ausentes; um rascunho de valores públicos foi preparado, sem salvar, pois ainda falta a senha SMTP. O editor do Cloudflare está aberto com uma proposta de SPF ainda não salva; ela foi preservada. Não considerar recebimento nem envio operacional até concluir DNS, instalar as credenciais no Render e testar.
...
| P1.10 | Geração paga, recibo e entrega por e-mail | **92%** | O domínio está autenticado no Brevo e o relay SMTP está disponível; `SMTP_PORT` e `SMTP_USE_TLS` existem no Render. O backend agora exige host, remetente, usuário e senha para qualquer envio autenticado, evitando tentativas com senha ausente. Faltam salvar os dados do relay, adicionar a chave SMTP como secret e comprovar recibo + currículo + carta em uma caixa confirmada. |
...
| P1.18 | Suporte ao cliente | **85% (estimativa)** | Central de ajuda, WhatsApps e chat flutuante público/autenticado estão publicados. Revalidação em 21/09 confirmou o botão na landing pública do Render, abertura do painel e resposta aprovada a uma pergunta sobre preços; os 9 testes específicos do chat passaram. A API aceita `message` (widget atual) e `question` (formato do exemplo de integração). Falta configurar MX/caixa de entrada e as quatro variáveis SMTP ausentes, depois confirmar o recebimento pelo canal de suporte. |
...
| P1.17 | Polimento visual residual | **98% (estimativa)** | Correções de candidaturas, segurança, configurações e grade responsiva dos preços estão Live; em 21/09 a produção confirmou alinhamento dos botões Start, Pro e Consultoria. Falta a revisão residual em telas autenticadas no navegador real. |
...
- A delegação do domínio `candidaturacerta.com.br` resolve para `frida.ns.cloudflare.com` e `todd.ns.cloudflare.com`; o apex aponta para os IPs do edge Render e `www` resolve para o hostname Render. HTTPS, landing, planos e presença do chat foram confirmados no domínio próprio. `APP_BASE_URL` foi aplicado pelo Blueprint no commit `c158c05`, depois confirmado **Live**; ainda falta revisar os callbacks/allowlists externos. O MX permanece nulo (`.`), logo o e-mail de contato não recebe mensagens. O painel Render atualmente tem `SMTP_PORT` e `SMTP_USE_TLS`, mas não tem `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME` nem `SMTP_PASSWORD`.
...
- O widget `/static/support-chat.js` e seu CSS são carregados na landing pública e no shell autenticado. A API pública `/api/support-chat` aplica limite de requisições, sanitiza a entrada, usa Gemini apenas para classificar em temas permitidos e devolve respostas do catálogo revisado; o cliente insere texto com `textContent`, sem renderizar HTML recebido. Em 21/09, o navegador confirmou o botão visível na landing do Render, abriu o painel e recebeu a resposta aprovada para “Quanto custa o plano Start?”; os 8 testes de `tests.test_support_chat` e `node --check app/static/support-chat.js` passaram. O SMTP/e-mail visível no widget continua pendente por falta de MX/caixa e credenciais de envio.
- Em 21/09, a auditoria somente dos nomes no ambiente do Render confirmou `SMTP_PORT` e `SMTP_USE_TLS`; `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME` e `SMTP_PASSWORD` não existem. Nenhum valor secreto foi exibido nem copiado. A etapa restante depende da configuração da caixa/MX e de credenciais do provedor de envio.
- Em 21/09, a conta Brevo mostrou `candidaturacerta.com.br` autenticado, relay SMTP ativo e uma chave SMTP ativa mascarada; nenhum segredo foi aberto. O Cloudflare mostra MX nulo e SPF salvo como `v=spf1 -all`; o editor contém uma proposta de atualização SPF não salva, preservada sem interação. Ainda não há caixa de entrada/MX nem variáveis SMTP completas no Render.
...
- Na validação ampla de 21/09, **355 testes unittest passaram**. O módulo `test_ats_registry_domains.py` não foi reexecutado neste runtime porque `pytest` não está instalado; sua execução anterior separada registrou 29/29. `node --check app/static/support-chat.js` e `git diff --check` também passaram.
- Na publicação atual, `/privacidade` retornou HTTP 200 no Render com as divulgações de fornecedores, IA e retenção visíveis. O endpoint público do chat aceita tanto `message` quanto `question`; os 9 testes do chat passaram. A execução unittest compatível passou 355 testes; a suíte exclusiva de domínios ATS depende de `pytest`, não disponível neste runtime.
- A inspeção visual pública de 21/09 em `/#planos` confirmou os botões de Start, Pro e Consultoria alinhados na grade de preços do Render. A pendência P1.17 continua limitada à revisão das telas autenticadas; nenhum dado da conta foi aberto nesta checagem.
...
| E-mails de Customer Success | Lembrete após 7 dias e resumo opcional ao registrar entrevista implementados com opt-in, outbox idempotente, retentativas e exclusão/portabilidade LGPD; envio real aguarda configuração SMTP. Boas-vindas duplicaria a confirmação do Supabase; outras etapas continuam manuais e expiração aguarda uma data confiável | **70%** | **P1.19**, sem duplicar o escopo de entrega paga |
| Entrega paga e histórico de documentos | Webhook enfileira geração no servidor; estado, lease, tentativas e retry são duráveis; os DOCX ficam em biblioteca owner-scoped, com download, retenção e exclusão. `7bc618a` está Live no Render; falta configurar `SMTP_*` e comprovar uma entrega real | **92%** | **P1.10 — concluir ao configurar SMTP** |
...
11. **Entrega pós-pagamento — P1.10:** `7bc618a` publica geração durável no servidor após webhook confirmado, além de recibo, persistência privada, histórico, download owner-scoped, e-mail dos dois DOCX, retenção, exclusão LGPD e retry de geração. Resta definir `SMTP_*` no Render e confirmar uma entrega real em Gmail/Outlook.
...
4. **P1.6 — portabilidade/exclusão LGPD concluída:** código, testes e exclusão definitiva de uma conta de teste em produção confirmados. O próximo bloqueio externo é P1.10: configurar variáveis SMTP no Render e testar uma entrega real de recibo/documentos em e-mail confirmado.
...
- Na inspeção de produção daquele registro histórico, o ambiente não tinha variáveis `SMTP_*`. A checagem mais recente, em 21/09/2026, encontrou `SMTP_PORT` e `SMTP_USE_TLS`, mas não `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME` nem `SMTP_PASSWORD`; o envio transacional continua pendente (**P1.10**).
...
| Recibo e documentos por e-mail | Recibo idempotente; os dois DOCX são anexados somente após geração, com destinatário confirmado, outbox, retry e biblioteca protegida | **P1.10 entregue no código**; falta SMTP de produção e teste real |

...
| Alertas “27 vagas abertas...” salvos como uma vaga | Corrigido no Gmail: alertas agrupados sem ao menos duas vagas identificáveis são ignorados como resumo, em vez de inventar uma oportunidade; o splitter mantém a separação por URLs/títulos e preserva LinkedIn/Indeed/Gupy como origem da vaga quando identificados | **P1.3 parcialmente entregue**; ampliar regras específicas por provedor quando houver amostras reais |
...
3. Formalizar heurísticas de RH brasileiro para triagem, ATS, pretensão salarial, regime e vaga fantasma; **a base já existe em `decision_engine`, `job_health` e no parser de captura**, com motivos explicáveis e testes parciais. Alertas Gmail agrupados sem vagas individuais agora são descartados como resumo, evitando filas artificiais; a plataforma detectada no alerta passa a ser a origem preservada da vaga, em vez de ficar genérica como Gmail/Outlook. Falta consolidar uma versão própria das heurísticas e ampliar casos por provedor antes de marcar concluído.

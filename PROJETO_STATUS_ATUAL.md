...
| IDOR e RLS obrigatório | Validação real avançada | Migração RLS e testes locais estão feitos; duas contas reais em produção retornaram somente seus próprios escopos na API de exportação, e compras pagas são vinculadas à candidatura exata. Falta apenas exercitar o download de arquivo pago com duas contas reais em **P0.1** |
...
- A API de exportação foi exercitada diretamente no contexto autenticado das duas sessões reais: a conta A recebeu HTTP 200 com apenas seu perfil, 1 vaga, 1 candidatura e 5 compras; a conta B recebeu HTTP 200 com perfil ausente e zero vagas, candidaturas e compras. Nenhum conteúdo pessoal foi registrado no relatório. O download de arquivo pago entre contas ainda permanece em **P0.1**.
...
| Isolamento/IDOR | Testes locais com dois usuários cobrem listagem, consulta, atualização e downloads; duas contas reais confirmaram o escopo da exportação JSON em produção | Código, testes locais, RLS e exportação real confirmados; falta somente o download de arquivo pago cruzado em **P0.1** |
...
| Suíte completa | Dependências de `requirements.txt` resolvidas em ambiente isolado; a suíte oficial em `tests/` executou 141 testes sem falhas | **Concluído nesta verificação** |

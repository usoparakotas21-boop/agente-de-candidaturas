...
**Git e produção:** a marca Candidatura Certa, o favicon, os contatos públicos e o chat de suporte estão publicados. O estado sincronizado mais recente de código e evidências está no Git; o Render confirmou como **Live** os deploys mais recentes, incluindo os registros de validação de 21/09/2026.
...
| P1.22 — prioridade máxima do produto | Candidatura automática por canal autorizado | **25% (estimativa)** | Perfil, análise, documentos direcionados, histórico e complemento Chrome 0.2.1 de preparação assistida já existem. **Envio automático continua em 0%:** não há conector oficial autorizado. Para submissão em nome do usuário faltam API/convênio e autorização de um empregador/ATS, além de consentimento por vaga/destino. O complemento bloqueia LinkedIn, Jobbol e Glassdoor e não envia formulários; nos demais portais, só pode ser usado onde o preenchimento assistido seja permitido. |
...
| P2.7+ | Domínio próprio, Kanban, exportação e extensão de navegador | **95% configuração do domínio (estimativa)** | Nameservers e DNS do domínio raiz/`www` resolvem, o site está acessível por HTTPS e `APP_BASE_URL` foi aplicado no Blueprint e está Live. Falta revisar os redirects/callbacks de Supabase, Google, Microsoft e Mercado Pago. O complemento Chrome 0.2.1 está implementado e empacotado; Kanban e exportação continuam no escopo de escala. |

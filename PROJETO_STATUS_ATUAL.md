...
**Commit de aplicação mais recente:** `d385cd8` — `Add explicit opt-in follow-up email digests` (GitHub atualizado; Render confirmou estado **Live**).
...
| P1 | Assinaturas mensais Start e Pro | **85% (estimativa)** | Start R$ 34,90/mês e Pro R$ 99/mês, cobrança recorrente, webhook e cancelamento têm código/testes; falta validar uma assinatura real e seu cancelamento. Nenhuma nova cobrança foi iniciada nesta rodada. |
...
| P1.17 | Polimento visual residual | **80%** | Componentes principais, onboarding, loading, OCR e edição do perfil estão publicados; resta a revisão visual dos fluxos menos usados. |
...
- **Assinaturas mensais Mercado Pago:** a landing e o backend definem Start a R$ 34,90/mês e Pro a R$ 99,00/mês, com preapproval mensal, webhook e cancelamento; os 7 testes de assinatura passaram. O ciclo ainda não foi exercitado com uma assinatura real em produção.
...
| Erros de validação orientados à ação | `/curriculos`, entrevistas e lookup do estúdio mantêm retry e respostas orientadas à ação; falta revisar fluxos assíncronos menos usados | **95%** | **P1.17**, revisão residual |

...
| Backup diário, restauração e revogação | Secret configurada; workflow diário e artifact cifrado por 30 dias; restore do archive em Supabase local descartável confirmou 11 tabelas, 527 linhas agregadas e 11 políticas RLS | Em **P0.11**, resta apenas confirmar cópia segura da chave privada fora deste computador |
...
11. **Backup e recuperação — restauração validada, alternativa gratuita em operação:** o run diário do GitHub Actions guarda artifact cifrado por 30 dias. Em 20/09, o archive foi restaurado com sucesso num stack oficial local do Supabase descartável; `pg_restore --exit-on-error` concluiu e a verificação agregada encontrou 11 tabelas públicas, 527 linhas e 11 políticas RLS. Os 19 testes direcionados passaram e os recursos locais temporários foram removidos. **P0.11 permanece parcial somente até o proprietário confirmar que guardou uma cópia da chave privada fora deste computador.** Sem upgrade do Supabase ou Render.
...
| Backup e recuperação | Workflow gratuito ativo; archive cifrado restaurado num stack oficial do Supabase local; 11 tabelas, 527 linhas agregadas e 11 políticas RLS conferidas | P0.11 parcial somente pela confirmação da cópia segura da chave fora do computador |
...
- Backups: o plano Free não inclui backup diário/PITR nativo. A alternativa sem custo adicional é `pg_dump` diário no GitHub Actions com artefato cifrado por 30 dias. A secret do banco está configurada, o run #4 concluiu e o restore foi validado em Supabase local descartável; **P0.11** aguarda somente confirmação de que a chave privada foi guardada em local seguro fora deste computador.
...
- **Backups automáticos:** usar GitHub Actions em vez de Render Cron ou
  Supabase Pro/PITR. Agendamento, secret, cifra, execução e restauração local
  Supabase-compatível estão confirmados; falta somente confirmar a cópia externa
  da chave privada.

...
| P1.17 | Polimento visual residual | **98% (estimativa)** | Correções de candidaturas, segurança, configurações e grade responsiva dos preços estão Live; em 21/09 a produção confirmou alinhamento dos planos, texto confuso de cota e um separador visual que fazia badges de match positivos parecerem negativos. A cota foi explicada; a API também limita scores inválidos a 0–100, e `d6ab263` removeu o separador indevido: o Render confirmou Live e a página autenticada `/vagas` mostrou 86% e 79% positivos. `e95caf3` ancorou o rodapé autenticado; `8e38a0a` posiciona o chat acima do rodapé quando este aparece, e o número de WhatsApp da landing foi confirmado livre de sobreposição em produção. Resta encerrar a revisão visual residual autenticada. |
...
- A inspeção da landing em `/#planos` confirmou os botões Start, Pro e Consultoria alinhados. A revisão autenticada de 21/09 passou por Dashboard, Configurações, Currículos e Banco de vagas; o badge de match foi publicado e conferido positivo em `/vagas`; resta encerrar a revisão visual residual.
...
| Refinamento de componentes, microinterações e consistência visual | Ações do cabeçalho, botões neutros, skeleton do drawer e da lista de candidaturas, retry e feedback de upload já foram tratados; falta a revisão visual dos fluxos menos usados | **90%** | **P1.17** |
...
| Erros de validação orientados à ação | `/curriculos`, entrevistas, lookup do estúdio e `/candidaturas` mantêm retry; falhas auxiliares de métricas/lembretes não ocultam mais a lista | **100% código nesta tela** | **P1.17** está Live; resta revisão visual residual |
...
17. **P1.17 — primeiro acesso e polimento, 98%:** o tour guiado de três passos salva o progresso por conta, retoma ao voltar de currículo/preferências e só dispensa por ação explícita. Entrevistas e a seleção/vínculo da vaga no estúdio têm skeleton, `aria-busy`, retry e estados vazios/erro; todos respeitam movimento reduzido. `/perfil` permite editar experiências, competências, formação e idiomas e preserva dados não alterados nas reimportações. Hover dos botões, pulso do status conectado, feedback de upload/OCR, skeleton/retry no Banco de vagas/drawer e favicon SVG exclusivo da Candidatura Certa já estão publicados. `/candidaturas` não some quando dados auxiliares falham; `/configuracoes` só habilita salvar depois de receber preferências válidas e mostra retry em erro; `/seguranca` recupera falhas de rede/timeout e anuncia o estado acessivelmente. `cc1595e` corrigiu o encaixe dos preços em larguras médias e está **Live** no Render; a grade foi conferida em prévia local. Nesta rodada, também foram executados cenários locais do simulador; falta a revisão visual residual em telas autenticadas no navegador real para marcar P1.17 como 100%.

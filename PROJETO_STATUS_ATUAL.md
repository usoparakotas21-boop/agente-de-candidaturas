...
- **P1.17 publicado no Render:** `e14086c` trouxe o tour de primeiro acesso com progresso por conta e estados assíncronos acessíveis; `414576f` acrescenta edição de experiências, competências, formação e idiomas, preservação das seções editadas e dos metadados nas reimportações, e correção para manter todas as linhas visíveis e salváveis. O Render confirmou `414576f` como **Live**; a suíte oficial atual passou em 281 testes (o módulo `test_ats_registry_domains.py` continua fora por depender de `pytest`, ausente no runtime empacotado).

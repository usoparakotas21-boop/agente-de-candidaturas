...
**Git e produção:** a marca Candidatura Certa, o favicon, os contatos públicos e o chat de suporte estão publicados; o commit `cf315b3` está sincronizado com `origin/master` e foi confirmado **Live** no Render.
**Produção:** `https://candidaturacerta.com.br` (o hostname Render continua disponível).
**Domínio próprio:** DNS agora resolve para a Cloudflare (`frida.ns.cloudflare.com`, `todd.ns.cloudflare.com`), os A do domínio raiz apontam para o edge do Render e `www` aponta para `agente-de-candidaturas.onrender.com`. O domínio principal respondeu por HTTPS e o chat de produção foi testado. Ainda falta confirmar no runtime do Render o `APP_BASE_URL` canônico e revisar os allowlists/callbacks externos; a alteração correspondente está preparada no Blueprint local. O MX continua nulo, portanto o endereço de contato não recebe e-mail.
...
| P1.18 | Suporte ao cliente | **85% (estimativa)** | Central de ajuda, WhatsApps e chat flutuante público/autenticado estão publicados. O chat consulta Gemini para classificar e responde com conteúdo aprovado, tem fallback local e rate limit; produção retornou HTTP 200. Falta tornar o e-mail operacional (MX/caixa e SMTP) e confirmar o recebimento pelo canal de suporte. |
...
| P2.7+ | Domínio próprio, Kanban, exportação e extensão de navegador | **90% configuração do domínio (estimativa)** | Nameservers e DNS do domínio raiz/`www` resolvem, e o domínio principal está acessível por HTTPS. Falta confirmar `APP_BASE_URL` no ambiente ativo e revisar os redirects/callbacks de Supabase, Google, Microsoft e Mercado Pago. Kanban, exportação e extensão continuam no escopo de escala. |
...
- A delegação do domínio `candidaturacerta.com.br` agora resolve para `frida.ns.cloudflare.com` e `todd.ns.cloudflare.com`; o apex aponta para os IPs do edge Render e `www` resolve para o hostname Render. HTTPS no domínio próprio e teste do chat em produção foram confirmados. Ainda é necessário confirmar o `APP_BASE_URL` no ambiente ativo e os callbacks dos provedores. O MX permanece nulo (`.`), logo o e-mail de contato não recebe mensagens; configurar MX/caixa e SMTP antes de considerar recibos/e-mails transacionais concluídos.

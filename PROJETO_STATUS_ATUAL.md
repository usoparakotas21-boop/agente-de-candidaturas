...
17. **UX de primeiro acesso e polimento final — entrega parcial publicada:** o dashboard agora oferece um tour guiado dispensável em três passos, com estado persistido no navegador e ações para currículos, preferências e captação. Também recebeu hover nos botões principais, pulso do status conectado, feedback de processamento na importação do currículo, skeleton/retry no Banco de vagas, mensagens de recuperação em currículo, preferências, candidaturas, entrevistas, perfil e configurações e respeito a `prefers-reduced-motion`. A tela de Currículos passou a agrupar o resultado do OCR por experiências, competências, formação e idiomas, o estúdio mostra progresso com spinner e `aria-busy` ao gerar a prévia, e o favicon `AC` foi publicado no shell público/autenticado; a revisão de estados assíncronos desta etapa está concluída.

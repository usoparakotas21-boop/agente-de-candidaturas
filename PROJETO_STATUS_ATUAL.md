...
17. **UX de primeiro acesso e polimento final — entrega parcial publicada:** o dashboard agora oferece um tour guiado dispensável em três passos, com estado persistido no navegador e ações para currículos, preferências e captação. Também recebeu hover nos botões principais, pulso do status conectado e respeito a `prefers-reduced-motion`. Ainda faltam a revisão da hierarquia dos dados extraídos por OCR, estados de progresso mais completos e a revisão final das mensagens de recuperação.

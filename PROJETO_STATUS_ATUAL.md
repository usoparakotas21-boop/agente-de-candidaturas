...
**Commit de aplicação mais recente:** `e14086c` — `Make onboarding resumable and async states accessible` (GitHub atualizado; Render confirmou estado **Live**).
...
- **P1.17 publicado no Render em `e14086c`:** o tour de primeiro acesso salva o progresso por ID da conta, pausa ao abrir currículo/preferências e retoma no retorno ao painel; somente “Pular guia” e o X concluem a dispensa. Entrevistas e a seleção de vaga no estúdio exibem skeleton acessível, retry e estados vazios/erro, respeitando `prefers-reduced-motion`. Validação: 27 testes do shell, 2 testes de feedback assíncrono e 6 cenários Node passaram; o Render marcou o commit como **Live**.
...
- **P1.10 entregue no código:** os DOCX pagos são persistidos como versões privadas em PostgreSQL, aparecem no histórico de `/curriculos`, têm downloads autenticados por proprietário, retenção e exclusão LGPD; a geração pós-retorno do Mercado Pago envia os dois arquivos ao e-mail confirmado quando SMTP está configurado. O outbox guarda estado/tentativas e oferece retry. Falta configurar SMTP no Render e confirmar uma entrega real.
...
| Entrega paga e histórico de documentos | Checkout de retorno gera e persiste os dois DOCX, cria biblioteca owner-scoped, downloads protegidos, e-mail confirmado com outbox/retry e retenção. Código publicado e confirmado no Render; falta SMTP de produção e teste real | **85%** | **P1.10 — concluir ao configurar SMTP** |
...
11. **Entrega pós-pagamento — P1.10:** código implementado para recibo, geração pós-retorno, persistência privada, histórico, download owner-scoped, e-mail dos dois DOCX com outbox/retry, retenção e exclusão LGPD. Resta definir `SMTP_*` no Render e confirmar uma entrega real em Gmail/Outlook.
...
17. **P1.17 — primeiro acesso e polimento:** o tour guiado de três passos já existia; agora o progresso fica namespaced pelo ID autenticado e retoma ao voltar de currículo/preferências, enquanto somente X/Pular concluem a dispensa. Entrevistas e a seleção/vínculo da vaga no estúdio têm skeleton, `aria-busy`, retry e estados vazios/erro; todos respeitam movimento reduzido. Já estavam publicados hover dos botões, pulso do status conectado, feedback de upload/OCR, skeleton e retry no Banco de vagas, drawer, mensagens de recuperação nas telas assíncronas, agrupamento do OCR e favicon `AC`. Os testes focados (27 shell, 2 de feedback e 6 Node) passaram; `e14086c` está **Live** no Render.

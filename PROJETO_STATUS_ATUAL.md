...
- **P1.10/P1.22 — reenvio de documentos:** o commit `58f973b` corrigiu o `KeyError: 'document-email-retry'` que dava HTTP 500, assim como os limites ausentes para prévia/envio de candidatura por e-mail. O commit `921a93d` está Live no Render e adicionou o caminho `SMTP_SSL` para a porta 465. O botão retornou HTTP 200, mas a tentativa controlada de `06:58:29` também expirou em `smtp_stage=connect` com `TimeoutError`, antes da autenticação; o log sanitizado registra somente fase, classe da exceção e código SMTP numérico opcional. Os testes não indicam senha inválida: a conexão de saída do Render ao relay não foi estabelecida. A próxima alternativa implementável é o envio HTTP da Brevo, sem expor a senha SMTP. **31 testes focados e 411 testes `unittest` passaram**; o módulo pytest-only `test_ats_registry_domains.py` foi excluído porque o runtime do projeto nega a inicialização e o runtime alternativo não tem a extensão nativa necessária para `pytest`. Nenhum e-mail foi confirmado nesta tentativa.

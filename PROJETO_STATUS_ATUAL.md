...
**Git e produção:** a marca Candidatura Certa, o favicon, os contatos públicos, o chat de suporte e o Copiloto Chrome 0.7.1 estão publicados. O recurso está em `8f85d1c`; o guia de instalação em `f0c64ce`; a linguagem mais simples do painel em `63e8feb`; os exemplos de callbacks de domínio próprio em `965dfbc`; esses commits estão **Live** no Render. A extensão faz análise contextual da vaga, deixa o Gemini desligado até a pessoa escolher, explica quais dados serão usados e oferece cópia rápida. A página Perfil explica a instalação em etapas e oferece ajuda. A checagem direta de `/health` foi bloqueada pelo navegador (`ERR_BLOCKED_BY_CLIENT`), então não alego validação HTTP nesta rodada.
...
| P1.22 — prioridade máxima do produto | Candidatura automática por canal autorizado | **40% (estimativa)** | Perfil, análise, documentos direcionados, histórico e Copiloto Chrome 0.7.1 autenticado, com botão persistente opcional por domínio, preparação, análise contextual da vaga, consentimento explicado em linguagem simples, sugestões Gemini com opt-in, cópia rápida e preenchimento consentido estão implementados e publicados. **Submissão automática continua em 0%:** não há conector oficial autorizado. Faltam API/convênio e autorização do empregador/ATS, além de consentimento por vaga/destino. O complemento não envia formulários nem contorna CAPTCHAs ou proteções; para cada portal, o autofill só pode ser usado quando permitido. |
| P1.10 | Geração paga, recibo e entrega por e-mail | **92%** | O domínio está autenticado no Brevo e o relay está pronto. `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME`, `SMTP_PORT` e `SMTP_USE_TLS` estão salvos no Render. Falta criar e digitar `SMTP_PASSWORD` no próprio Render e comprovar recibo + currículo + carta em uma caixa confirmada. |

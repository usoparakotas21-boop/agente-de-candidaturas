...
3. Formalizar heurísticas de RH brasileiro para triagem, ATS, pretensão salarial, regime e vaga fantasma; **a base já existe em `decision_engine`, `job_health` e no parser de captura**, com motivos explicáveis e testes parciais. Alertas Gmail agrupados sem vagas individuais agora são descartados como resumo, evitando filas artificiais. Falta consolidar uma versão própria das heurísticas e ampliar casos por provedor antes de marcar concluído.
...
| Suíte completa | Dependência `psycopg[binary]` instalada no ambiente local; descoberta completa executou 132 testes | **Concluído nesta verificação** |

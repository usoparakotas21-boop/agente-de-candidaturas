...
- A exportação/portabilidade JSON owner-scoped está disponível na área de Segurança. **P1.6 corrigido no código:** a exclusão inclui assinaturas, relações/documentos e demais registros do proprietário; confirma o commit local antes de remover a identidade Supabase, aceita 404 como retry idempotente e interrompe a exclusão se um arquivo legado privado não puder ser removido. Os testes isolados cobrem proprietário A/B e falhas simuladas; nenhum usuário real foi excluído. Falta a prova operacional com conta descartável dedicada. Não há bucket de Storage ativo.
...
6. **P1.6 implementado e testado:** a exportação JSON é owner-scoped; a exclusão definitiva remove todos os modelos do usuário incluindo `BillingSubscription`, mantém as contas alheias, faz purge local antes da remoção da identidade no Supabase, trata 404 como sucesso de retry e preserva acesso se falhar commit ou unlink de arquivo. **7 testes direcionados passam.** Para fechar a evidência operacional resta executar o fluxo com conta descartável confirmada; não usar conta real existente. Não há bucket de Storage ativo.
...
4. **P1.6 — portabilidade/exclusão LGPD:** código e testes concluídos; a evidência final em produção requer uma conta descartável confirmada e dedicada. O próximo bloqueio externo é P1.10: configurar variáveis SMTP no Render e testar uma entrega real de recibo/documentos em e-mail confirmado.

...
**Commit publicado:** `2c10c9b` — `feat: improve application recovery feedback`
...
| Feedback assíncrono de OCR e validação de arquivos | Mensagens específicas, região de status acessível, skeleton do drawer, skeleton/retry do Banco de vagas e recuperação explícita em currículo, preferências e candidaturas existem; resta apenas revisar estados de ciclo de vida e cópia final | **95%** | **P1.17** |
...
17. **UX de primeiro acesso e polimento final — entrega parcial publicada:** o dashboard agora oferece um tour guiado dispensável em três passos, com estado persistido no navegador e ações para currículos, preferências e captação. Também recebeu hover nos botões principais, pulso do status conectado, feedback de processamento na importação do currículo, skeleton/retry no Banco de vagas, mensagens de recuperação em currículo, preferências e candidaturas e respeito a `prefers-reduced-motion`. A tela de Currículos passou a agrupar o resultado do OCR por experiências, competências, formação e idiomas, o estúdio mostra progresso com spinner e `aria-busy` ao gerar a prévia, e o favicon `AC` foi publicado no shell público/autenticado; ainda falta revisar estados de ciclo de vida e a cópia final de recuperação.

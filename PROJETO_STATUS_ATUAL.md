...
| MFA | Enrollment/status/unenroll e challenge/verify TOTP; login com fator verificado cria desafio temporário, conclusão promove a sessão e logout revoga sessão pendente; duas contas reais exibem estados distintos (`enabled=false` e `enabled=true`), e um desafio ausente retornou 401 estável | Código e interface publicados; permanece o login E2E com código real, recuperação e expiração em **P0.3** |
...
| Suíte completa | Dependências de `requirements.txt` resolvidas em ambiente isolado; a suíte oficial em `tests/` executou 142 testes sem falhas | **Concluído nesta verificação** |

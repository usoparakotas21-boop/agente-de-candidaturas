...
3. **MFA opcional por conta — correção publicada no Render (`9dfcaff`):** após senha correta, o backend verifica fatores TOTP confirmados; quem ativou MFA precisa concluir o desafio antes de receber os cookies de sessão, enquanto quem não tem fator entra normalmente. A exigência global foi removida do código e do Blueprint. Os testes cobrem os dois caminhos, cookies pendentes, código inválido e promoção da sessão. Falta confirmar um login real com TOTP; recuperação e expiração precisam de regressão própria antes de encerrar **P0.3**.
...
| MFA | Enrollment/status/unenroll e challenge/verify TOTP; correção publicada exige desafio para cada fator verificado e mantém login normal sem fator; testes cobrem challenge, cookies temporários, código inválido e promoção | Deploy `9dfcaff` está Live; falta login real com TOTP e regressão de recuperação/expiração para fechar **P0.3**; MFA permanece opcional por conta |

...
| P1.17 | Polimento visual residual | **98% (estimativa)** | Correções de candidaturas, segurança, configurações e grade responsiva dos preços estão Live; em 21/09 a produção confirmou alinhamento dos planos, texto confuso de cota e um separador visual que fazia badges de match positivos parecerem negativos. A cota foi explicada; a API também limita scores inválidos a 0–100, e o CSS separa o badge sem alterar dados armazenados. Falta publicar e verificar o novo visual e encerrar a revisão residual autenticada. |
...
- Na publicação atual, `/privacidade` retornou HTTP 200 no Render com as divulgações de fornecedores, IA e retenção visíveis. O endpoint público do chat aceita `message` e `question`, com 9 testes específicos aprovados. A cota excedida da tela Configurações agora tem correção local coberta por teste e aguarda deploy; a biblioteca de documentos confirmou a orientação de armazenamento e o bloqueio de e-mail sem endereço confirmado.
- A inspeção da landing em `/#planos` confirmou os botões Start, Pro e Consultoria alinhados. A revisão autenticada de 21/09 passou por Dashboard, Configurações, Currículos e Banco de vagas; a pendência P1.17 é publicar e conferir o espaçamento/cores do badge de match e encerrar a revisão residual.

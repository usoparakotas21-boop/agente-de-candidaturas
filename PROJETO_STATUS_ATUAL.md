...
**Git e produção:** a marca Candidatura Certa, o favicon, os contatos públicos e o chat de suporte estão publicados. O commit `f31c3cc` está **Live** no Render. A checagem atual retornou `/health` 200 com banco conectado, as seis rotas do Copiloto no OpenAPI e o pacote Chrome atualizado com 22.298 bytes.
...
| P2.7+ | Domínio próprio, Kanban, exportação e extensão de navegador | **95% configuração do domínio (estimativa)** | Nameservers e DNS do domínio raiz/`www` resolvem, o site está acessível por HTTPS e `APP_BASE_URL` está no Render. Falta revisar os redirects/callbacks de Supabase, Google, Microsoft e Mercado Pago. O Copiloto Chrome 0.5.0 está Live; a versão 0.6.0, com botão automático por domínio autorizado, foi construída e testada localmente e aguarda este deploy. Continua distribuído por ZIP para instalação manual; Chrome Web Store ainda não publicado. Kanban e exportação continuam no escopo de escala. |
...
- **P1.22 — Copiloto 0.6.0 (aguardando publicação):** o complemento agora pode registrar um botão flutuante persistente somente no domínio ATS que a pessoa habilitar. O Chrome solicita acesso opcional ao host específico; desligar o recurso revoga o registro e a permissão. O detector lê URL e texto visível localmente e só revela dados do perfil após clique e consentimento. A revisão manual continua obrigatória. O pacote ZIP contém 12 arquivos e passou 11 testes do complemento, 14 testes Python de Copiloto/e-mail e verificações de sintaxe/manifesto; o Render ainda serve a versão 0.5.0 até concluir o próximo deploy.

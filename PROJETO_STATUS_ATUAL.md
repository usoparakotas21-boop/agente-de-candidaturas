...
| P1.22 | Candidatura automática por canal autorizado | **15% (estimativa)** | Perfil, análise, currículo/carta direcionados e histórico já existem. O fluxo de Vagas dispara a preparação dos documentos em um clique e `87f542f` está Live; ainda não há integração de envio ou autorização de candidatura em nenhum portal. A submissão automática exige API/convênio permitido por plataforma; extensão/bot não será usado onde os termos o proíbem. |
...
- **P1.20 — nove fontes prioritárias e gate endurecido:** LinkedIn, InfoJobs, Vagas.com, Sólides, Jobbol, Catho, Gupy, Empregos.com.br e Glassdoor foram pesquisados e registrados com decisão por fonte em `SOURCE_GOVERNANCE.md`; nenhuma das nove tem autorização arquivada. Os 30 domínios conhecidos pelo classificador têm estado explicitamente bloqueado. A verificação padrão exige direitos para coleta automática e exibição comercial; usos adicionais também devem estar no escopo. Os 14 testes de governança e três verificações diretas de domínio passaram; o módulo parametrizado de ATS depende de `pytest`, ausente no runtime disponível. Nenhuma coleta automática foi iniciada.

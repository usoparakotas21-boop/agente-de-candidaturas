...
| P1.17 | Polimento visual residual | **98% (estimativa)** | Correções de candidaturas, segurança, configurações e grade responsiva dos preços estão Live; em 21/09 a produção confirmou alinhamento dos botões Start, Pro e Consultoria e expôs uma mensagem confusa de cota excedida. A correção local explica acesso às vagas salvas e a data de renovação; falta publicar e verificar a tela atualizada e terminar a revisão residual das telas autenticadas. |
...
- A revisão autenticada de `/configuracoes` encontrou o contador “93 de 30 oportunidades usadas” sem explicação, embora as vagas existentes continuassem disponíveis. `configuracoes.html` agora mostra que os itens salvos permanecem acessíveis, informa a próxima data de liberação e aponta para as opções de plano; foi adicionada regressão automatizada. A mensagem ainda aguarda deploy e confirmação em produção.
...
- Na validação compatível mais recente, **359 testes unittest passaram**. O módulo `test_ats_registry_domains.py` depende de `pytest`, indisponível neste runtime; sua execução separada anterior registrou 29/29. Os scripts inline de Configurações e do widget passaram `node --check`; `git diff --check` passou.
...
- A inspeção da landing em `/#planos` confirmou os botões Start, Pro e Consultoria alinhados. A revisão autenticada de 21/09 passou por Dashboard, Configurações e Currículos; a pendência P1.17 agora é publicar a explicação de cota e encerrar a revisão residual.

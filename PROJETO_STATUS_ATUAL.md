...
**Domínio próprio:** DNS resolve para a Cloudflare (`frida.ns.cloudflare.com`, `todd.ns.cloudflare.com`), os A do domínio raiz apontam para o edge do Render e `www` aponta para `agente-de-candidaturas.onrender.com`. O domínio principal respondeu por HTTPS; `APP_BASE_URL` canônico foi aplicado pelo Blueprint e o commit `c158c05` está **Live**. Restam os allowlists/callbacks dos provedores externos. O MX continua nulo, portanto o endereço de contato não recebe e-mail.
**E-mail de contato:** o endereço foi publicado na interface; o SPF agora autoriza o relay Brevo (`v=spf1 include:spf.brevo.com ~all`) e a resolução pública foi confirmada. O MX permanece nulo (`.`), portanto não há caixa de entrada/recebimento. O Render tem `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME`, `SMTP_PORT=587` e `SMTP_USE_TLS=true`; falta `SMTP_PASSWORD`, que o proprietário deve criar e digitar diretamente no Render. Não considerar envio operacional até inserir essa chave e testar.
...
| P1.18 | Suporte ao cliente | **85% (estimativa)** | Central de ajuda, WhatsApps e chat flutuante público/autenticado estão publicados. A base do chat foi ampliada para funcionamento, primeiros passos, score, ATS, pagamentos, arquivos, biblioteca, entrevistas, conta e falhas comuns; há quatro perguntas rápidas clicáveis e link para `/ajuda`. Se Gemini classificar uma pergunta conhecida como desconhecida, o servidor tenta a correspondência local antes do fallback humano. Os 11 testes do chat passaram. A API aceita `message` e `question`. Faltam caixa de entrada/MX e `SMTP_PASSWORD`, depois confirmar o recebimento pelo canal de suporte. |
...
| P1.17 | Polimento visual residual | **98% (estimativa)** | Correções de candidaturas, segurança, configurações e grade responsiva dos preços estão Live; em 21/09 a produção confirmou alinhamento dos planos, texto confuso de cota e um separador visual que fazia badges de match positivos parecerem negativos. A cota foi explicada; a API também limita scores inválidos a 0–100, e `d6ab263` removeu o separador indevido: o Render confirmou Live e a página autenticada `/vagas` mostrou 86% e 79% positivos. Resta encerrar a revisão visual residual autenticada. |
...
- Após o commit `7891502` ficar **Live**, foram salvos no Render os valores `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME`, `SMTP_PORT=587` e `SMTP_USE_TLS=true`. A senha não foi visualizada nem digitada; `SMTP_PASSWORD` continua para o proprietário criar e inserir. Depois, o SPF foi atualizado no Cloudflare para autorizar o Brevo e a consulta DNS pública confirmou o novo TXT. O MX segue nulo.
...
- A delegação do domínio `candidaturacerta.com.br` resolve para `frida.ns.cloudflare.com` e `todd.ns.cloudflare.com`; o apex aponta para os IPs do edge Render e `www` resolve para o hostname Render. HTTPS, landing, planos e presença do chat foram confirmados no domínio próprio. `APP_BASE_URL` foi aplicado pelo Blueprint no commit `c158c05`, depois confirmado **Live**; ainda falta revisar os callbacks/allowlists externos. O MX permanece nulo (`.`), logo o e-mail de contato não recebe mensagens. Atualização posterior de 21/09: `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME`, `SMTP_PORT=587` e `SMTP_USE_TLS=true` foram salvos no Render; falta somente `SMTP_PASSWORD` e a prova de entrega.
...
- O widget `/static/support-chat.js` e seu CSS são carregados na landing pública e no shell autenticado. A API pública `/api/support-chat` aplica limite de requisições, sanitiza a entrada, usa Gemini apenas para classificar em temas permitidos e devolve respostas do catálogo revisado; o cliente insere texto com `textContent`, sem renderizar HTML recebido. Em 21/09, o navegador confirmou o botão visível na landing do Render, abriu o painel e recebeu a resposta aprovada para “Quanto custa o plano Start?”; os 9 testes de `tests.test_support_chat` e `node --check app/static/support-chat.js` passaram. O SMTP/e-mail visível no widget continua pendente por falta de MX/caixa e credenciais de envio.
...
- Em 21/09, a conta Brevo mostrou `candidaturacerta.com.br` autenticado, relay SMTP ativo e uma chave SMTP ativa mascarada; nenhum segredo foi aberto. O Cloudflare mostra MX nulo e SPF salvo como `v=spf1 include:spf.brevo.com ~all`; a consulta pública do DNS confirmou esse registro. O MX/caixa de entrada e o teste de entrega permanecem pendentes.
...
- Na publicação atual, `/privacidade` retornou HTTP 200 no Render com as divulgações de fornecedores, IA e retenção visíveis. O endpoint público do chat aceita `message` e `question`; agora inclui atalhos de dúvidas comuns, novas respostas revisadas e fallback local quando Gemini retorna `unknown`, com 11 testes específicos aprovados. A cota excedida da tela Configurações foi publicada em `77f7a77` e confirmada no Render; a biblioteca de documentos confirmou a orientação de armazenamento e o bloqueio de e-mail sem endereço confirmado.

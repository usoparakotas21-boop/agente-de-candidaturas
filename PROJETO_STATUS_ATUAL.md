...
3. **MFA opcional por conta — correção implementada localmente:** após senha correta, o backend verifica fatores TOTP confirmados; quem ativou MFA precisa concluir o desafio antes de receber os cookies de sessão, enquanto quem não tem fator entra normalmente. A exigência global foi removida do código e do Blueprint. Os testes locais cobrem os dois caminhos, cookies pendentes, código inválido e promoção da sessão. Falta publicar e confirmar um login real com TOTP; recuperação e expiração precisam de regressão própria antes de encerrar **P0.3**.
...
| MFA | Enrollment/status/unenroll e challenge/verify TOTP; correção local exige desafio para cada fator verificado e mantém login normal sem fator; testes cobrem challenge, cookies temporários, código inválido e promoção | Publicação e login real com TOTP ainda pendentes em **P0.3**; MFA permanece opcional por conta |
...
- `MFA_LOGIN_ENFORCE` foi removida da aplicação, do `.env.example` e do `render.yaml`; a MFA não é imposta a quem não a ativou. Os sete testes de `tests/test_mfa_flow.py` passaram; falta publicar e validar o código real numa conta de teste.

...
**Git e produção:** a marca Candidatura Certa, favicon próprio e contatos públicos do commit `979c5c5` estão **Live** no Render. O endereço Render continua servindo o site.
**Produção:** `https://agente-de-candidaturas.onrender.com`  
**Domínio próprio:** `candidaturacerta.com.br` já foi associado ao serviço no Render via Blueprint. Os CNAMEs raiz e `www` apontam para `agente-de-candidaturas.onrender.com` no Cloudflare e SSL/TLS está em Full. A zona ainda está pendente porque o Registro.br mostra `frida.ns.cloudflare.com` e `todd.ns.cloudflare.com` em transição; faltam propagação, verificação/emissão do certificado e configuração do endereço base/callbacks para o novo domínio.
...
| P1.18 | Suporte ao cliente | **60% (estimativa)** | Central de ajuda e WhatsApps de suporte/consultoria estão publicados; o e-mail está visível, mas o DNS atual não aceita recebimento. Configurar o provedor de e-mail e testar ambos os canais. |
...
| P2.7+ | Domínio próprio, Kanban, exportação e extensão de navegador | **75% configuração do domínio (estimativa)** | Associação no Render, CNAMEs raiz/www e SSL/TLS Full estão configurados; falta a propagação dos nameservers, verificar o certificado, atualizar o endereço base e callbacks. Kanban, exportação e extensão continuam no escopo de escala. |
...
- `979c5c5` publicou a marca **Candidatura Certa**, o favicon SVG exclusivo, contatos atualizados na landing page, ajuda, configurações, páginas legais e comprovante, e links separados para suporte e consultoria. A landing de produção foi recarregada e confirmou título, marca, contatos e preços. O hostname do Render foi mantido durante a transição.
- A zona `candidaturacerta.com.br` na Cloudflare ainda está **Pending**, aguardando a propagação dos nameservers do Registro.br. Os CNAMEs raiz e `www` já apontam para o Render em modo DNS only, sem registros AAAA; o SSL/TLS está em Full. Há registros de validação/DKIM do Brevo, mas o MX é nulo (`.`) e o SPF está como `-all`, então não considerar recebimento de e-mail operacional. Após ativação, verificar o domínio e certificado no Render e atualizar o endereço base/callbacks da aplicação.

...
| P1.18 | Suporte ao cliente | **85% (estimativa)** | Central de ajuda, WhatsApps e chat flutuante público/autenticado estão publicados. Revalidação em 21/09 confirmou o botão na landing pública do Render, abertura do painel e resposta aprovada a uma pergunta sobre preços; os 9 testes específicos do chat passaram. A API aceita `message` (widget atual) e `question` (formato do exemplo de integração). Faltam caixa de entrada/MX e `SMTP_PASSWORD`, depois confirmar o recebimento pelo canal de suporte. |
...
| Central de ajuda e FAQ | FAQ da landing em **P1.16**; a central pública `/ajuda` cobre formatos, privacidade, pagamentos avulsos e recorrentes, cancelamento, entrega dos documentos e simulação com IA/local | **90%** | **P1.18** |

...
- A rota do Mercado Pago é pública, consulta o provedor e faz transição idempotente para `PAID`; o endpoint de produção e o evento Pagamentos estão configurados no painel, `MERCADOPAGO_WEBHOOK_SECRET` está presente no Render e o deploy está Live. O retorno agora restaura a candidatura no estúdio e gera os dois arquivos após a confirmação; ainda falta executar replay/checkout controlado em produção para fechar a evidência externa.
...
- O runbook `DISASTER_RECOVERY.md` documenta backup, restauração isolada e revogação/rotação emergencial. Em 19/09/2026 foi criado um dump real do Supabase PostgreSQL 17, validado, restaurado parcialmente em cluster local descartável e depois criptografado com AES-256-GCM/RSA-OAEP; o original em texto puro foi apagado. A cópia e a chave privada estão em `backups/`, ignoradas pelo Git e com ACL Windows restrita. O workflow diário gratuito do GitHub Actions está preparado com artefato cifrado por 30 dias; falta configurar a secret do banco, guardar a chave privada fora deste computador, validar uma execução e restaurar integralmente em alvo Supabase-compatível.
...
| Backup diário, restauração e revogação | Dump real validado, recuperação filtrada testada em PostgreSQL 17 isolado e workflow gratuito com upload criptografado preparado | Falta configurar a secret do banco, salvar a chave privada fora do computador, confirmar a primeira execução e validar restauração integral Supabase-compatível em **P0.11** |
...
- O gate do Mercado Pago avançou: a suíte direcionada de webhook, armazenamento privado e exportação passou em **18 testes**, `/health` retornou 200 e o callback público de sucesso retornou 303 para `/criar-documentos?application_id=15&payment_status=approved`. A sessão com compra `PAID` existente exibiu os botões de currículo e carta gerados. Continua pendente somente uma nova transação controlada e o replay real no provedor para fechar a evidência externa do **P0.5**.
...
| Backup e recuperação | Dump real validado; restauração filtrada aprovada; workflow gratuito configurado para artefatos cifrados com retenção de 30 dias | P0.11 parcial; falta a secret do banco, cópia segura da chave privada, primeiro run e restauração integral |
...
- Backups: o plano Free não inclui backup diário/PITR nativo. A alternativa sem custo adicional é `pg_dump` diário no GitHub Actions com artefato cifrado por 30 dias; **P0.11** aguarda a secret do banco, a cópia externa da chave privada, o primeiro run e a restauração integral com Supabase local/compatível.
...
- O dump foi criptografado com AES-256-GCM/RSA-OAEP após o teste, e o arquivo em
  ...
  30 dias; ele permanece inativo até a secret `SUPABASE_DATABASE_URL` ser
  cadastrada. A chave privada também precisa ser guardada num gerenciador seguro
  externo para permitir recuperação se este computador for perdido.

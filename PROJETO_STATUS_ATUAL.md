...
**Git e produção:** esta atualização inclui a correção de `/candidaturas`; o último commit de aplicação confirmado como Live antes desta atualização era `d385cd8`.
...
| P1.17 | Polimento visual residual | **90% (estimativa)** | Corrigi a tela de candidaturas para manter a lista disponível quando métricas ou lembretes falham, e incluí skeleton e retry acessível. Falta conferir em produção e concluir a revisão visual dos fluxos menos usados. |
...
- **P1.17 — correção adicional preparada:** `/candidaturas` agora carrega primeiro a lista principal; falhas nas métricas ou nos lembretes ficam isoladas, com estado acessível e retry próprio. A lista inicial/retry usa skeleton, e três cenários simulados cobrem falha independente de métricas, falha de lembretes e indisponibilidade da lista.

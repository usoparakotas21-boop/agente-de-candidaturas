...
| P1.17 | Polimento visual residual | **98% (estimativa)** | Correções de candidaturas, segurança, configurações e grade responsiva dos preços estão Live; em 21/09 a produção confirmou alinhamento dos planos e encontrou texto confuso de cota excedida e scores negativos de match em registros antigos. A cota foi explicada e a API agora limita scores exibidos ao intervalo de 0–100, com badges verdes/âmbar/neutros e sem alterar o dado armazenado; falta publicar, conferir `/vagas` e encerrar a revisão residual autenticada. |
...
- A revisão autenticada de `/configuracoes` encontrou o contador de cota excedida sem explicação; `configuracoes.html` agora informa que os itens salvos permanecem acessíveis, a data da renovação e as opções de plano. Em `/vagas`, registros antigos apareceram com match negativo; a serialização das rotas de vagas, candidaturas e entrevistas agora apresenta score de 0 a 100, e os badges usam faixas verde/âmbar/neutra. Nada é regravado no banco. Os testes de regressão passaram; esta correção aguarda commit/deploy e verificação em produção.
...
- A inspeção da landing em `/#planos` confirmou os botões Start, Pro e Consultoria alinhados. A revisão autenticada de 21/09 passou por Dashboard, Configurações, Currículos e Banco de vagas; a pendência P1.17 é publicar e verificar o score de match normalizado e encerrar a revisão residual.

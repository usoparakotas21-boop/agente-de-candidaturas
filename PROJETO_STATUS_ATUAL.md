...
- `25c813e` corrigiu a sobreposição do botão do chat com o WhatsApp no rodapé público ao reservar área inferior segura; o link foi conferido visualmente no domínio próprio com espaço livre. O Render confirmou o deploy **Live**. `tests.test_support_chat` passou **12 testes**, `node --check app/static/support-chat.js` e `git diff --check` passaram.

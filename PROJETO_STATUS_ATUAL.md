...
**Commit publicado:** `32d0a06` — `feat: improve jobs loading feedback`
...
| Feedback assíncrono de OCR e validação de arquivos | Mensagens específicas, região de status acessível, skeleton do drawer e skeleton/retry do Banco de vagas existem; ainda faltam estados equivalentes em algumas telas de preferências e ciclo de vida | **85%** | **P1.17** |
...
17. **UX de primeiro acesso e polimento final — entrega parcial publicada:** o dashboard agora oferece um tour guiado dispensável em três passos, com estado persistido no navegador e ações para currículos, preferências e captação. Também recebeu hover nos botões principais, pulso do status conectado, feedback de processamento na importação do currículo, skeleton/retry no Banco de vagas e respeito a `prefers-reduced-motion`. A tela de Currículos passou a agrupar o resultado do OCR por experiências, competências, formação e idiomas, o estúdio mostra progresso com spinner e `aria-busy` ao gerar a prévia, e o favicon `AC` foi publicado no shell público/autenticado; ainda faltam estados equivalentes em algumas telas de preferências e ciclo de vida e a revisão final das mensagens de recuperação.

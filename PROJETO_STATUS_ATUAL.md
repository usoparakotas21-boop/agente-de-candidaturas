...
**Git e produção:** a marca Candidatura Certa, o favicon, os contatos públicos, o chat de suporte e o Copiloto Chrome 0.7.0 estão publicados. O commit `8f85d1c` está **Deploy succeeded** no Render. A versão 0.7.0 acrescenta análise contextual da vaga no painel lateral, sugestões opcionais do Gemini e cópia rápida; ZIP local atualizado. A checagem direta de `/health` foi bloqueada pelo navegador (`ERR_BLOCKED_BY_CLIENT`), então não alego validação HTTP nesta rodada.

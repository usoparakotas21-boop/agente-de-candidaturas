...
5. **Webhook Mercado Pago — código reforçado, configuração a reconferir:** endpoint de produção, evento Pagamentos e `MERCADOPAGO_WEBHOOK_SECRET` estão registrados; o código rejeita valor divergente e moeda diferente de BRL, exige HMAC/replay/idempotência e agora retorna 503 quando o Access Token não está disponível, ou 401 para assinatura inválida. Uma chamada sem assinatura feita antes deste deploy retornou `verified:false`, evidência de que o token precisa ser reconfirmado no ambiente publicado. Falta replay/checkout controlado.
...
| Webhook de pagamento | Rota pública do Mercado Pago valida `x-signature`/`x-request-id` com HMAC e janela de replay; sem Access Token retorna 503 e sem assinatura retorna 401; o segredo aparece mascarado no Render | Código publicado; reconfirmar `MERCADOPAGO_ACCESS_TOKEN` no Render e executar replay/checkout controlado em **P0.5** |

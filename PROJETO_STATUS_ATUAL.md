...
**E-mail de contato:** o endereço foi publicado na interface; Cloudflare contém MX do Zoho (`mx.zoho.com`, `mx2.zoho.com`, `mx3.zoho.com`) e o resolvedor público 1.1.1.1 confirma os três registros. O SPF publicado autoriza o relay Brevo (`v=spf1 include:spf.brevo.com ~all`). A entrada está configurada, mas a entrega em caixa ainda não foi testada. No Render, `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME`, `SMTP_PORT=587` e `SMTP_USE_TLS=true` estão presentes; `SMTP_PASSWORD` não está configurada. Não considerar envios do aplicativo operacionais até inserir essa senha e validar recibo, documentos e notificações.
...
| P1.10 | Geração paga, recibo e entrega por e-mail | **92%** | O domínio está autenticado no Brevo e o relay está pronto. `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME`, `SMTP_PORT` e `SMTP_USE_TLS` estão salvos no Render. A Brevo mostra uma chave SMTP ativa, mas oculta o valor; a conta exige confirmação por e-mail para gerar uma nova. Falta o proprietário concluir a confirmação/geração e inserir `SMTP_PASSWORD` diretamente no Render, depois comprovar recibo + currículo + carta em uma caixa confirmada. |
...
- **P1.10/P1.19/P1.18 — SMTP:** revalidei o Render sem revelar valores; `SMTP_PASSWORD` está ausente e o campo da variável está aberto, pronto para receber o segredo. A Brevo usa `smtp-relay.brevo.com:587`, autenticação no usuário SMTP cadastrado, e exibe uma chave ativa mascarada, sem opção de recuperar o valor. A solicitação de novo código de confirmação por e-mail foi acionada; o proprietário precisa confirmar a identidade na Brevo, criar uma chave nova e colar seu valor diretamente no campo `SMTP_PASSWORD` do Render. Nenhum segredo foi copiado para o chat. Ainda faltam deploy após salvar e teste real de recibo/documentos/notificações.

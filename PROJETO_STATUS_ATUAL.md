...
**Git e produção:** a marca Candidatura Certa, o favicon, os contatos públicos e o chat de suporte estão publicados; o commit `c158c05` está sincronizado com `origin/master` e foi confirmado **Live** no Render.
...
**E-mail de contato:** o endereço foi publicado na interface; os registros DNS atuais mostram MX nulo (`.`) e SPF `-all`, sem provedor de e-mail configurado. Não considerar o recebimento de mensagens operacional até configurar MX/SPF/DKIM no provedor e testar.
...
| P1.10 | Geração paga, recibo e entrega por e-mail | **92%** | Configurar SMTP no Render e comprovar recibo + currículo + carta em uma caixa confirmada. |
...
| P1.19 | E-mails de ciclo de vida | **70% (estimativa)** | Follow-up após 7 dias e avisos de entrevista têm opt-in, outbox privada, retries, agrupamento diário/semanal e retenção de 60 dias. Falta configurar SMTP no Render e validar envio real em caixa confirmada; aviso de expiração fica indisponível até haver uma data de vencimento confiável. |
...
- A delegação do domínio `candidaturacerta.com.br` resolve para `frida.ns.cloudflare.com` e `todd.ns.cloudflare.com`; o apex aponta para os IPs do edge Render e `www` resolve para o hostname Render. HTTPS, landing, planos e presença do chat foram confirmados no domínio próprio. `APP_BASE_URL` foi aplicado pelo Blueprint e o commit `c158c05` está **Live**; ainda falta revisar os callbacks/allowlists externos. O MX permanece nulo (`.`), logo o e-mail de contato não recebe mensagens; configurar MX/caixa e SMTP antes de considerar recibos/e-mails transacionais concluídos.

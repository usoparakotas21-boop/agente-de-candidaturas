...
| P1.19 | E-mails de ciclo de vida | **70% (estimativa)** | Follow-up após 7 dias e avisos de entrevista têm opt-in, outbox privada, retries, agrupamento diário/semanal e retenção de 60 dias. A configuração comum do relay falha fechada sem credenciais completas. Falta `SMTP_PASSWORD` e validar envio real em caixa confirmada; aviso de expiração fica indisponível até haver uma data de vencimento confiável. |
...
| P1.22 | Candidatura automática por canal autorizado | **15% (estimativa)** | Perfil, análise, currículo/carta direcionados e histórico já existem. O fluxo de Vagas prepara os documentos em um clique (`87f542f` Live). **Nenhuma candidatura é enviada automaticamente hoje:** falta API/convênio e autorização específica por empregador/ATS, além de consentimento explícito da pessoa para cada destino/vaga. A permissão de captar/exibir vaga não dá direito de enviar candidatura; a governança agora exige o uso separado `automated_submission`. Extensão/bot não será usado onde os termos o proíbem. |

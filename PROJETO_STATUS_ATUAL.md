...
- **P1.10/P1.19/P1.18 — configuração SMTP atualizada:** a lista de variáveis no Render foi recarregada sem abrir valores. `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME`, `SMTP_PORT` e `SMTP_USE_TLS` estão presentes; `SMTP_PASSWORD` está ausente. Portanto não foi feito envio real de recibo, documento ou ciclo de vida.
...
- **Checkpoint posterior — 21/09/2026:** o Cloudflare agora mostra os MX Zoho `mx.zoho.com` (prioridade 10), `mx2.zoho.com` (20) e `mx3.zoho.com` (50); `Resolve-DnsName` contra 1.1.1.1 confirmou os três. A caixa de entrada ainda não recebeu um e-mail de teste nesta validação. O Render mostra `SMTP_FROM_EMAIL`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_USE_TLS` e `SMTP_USERNAME`, mas não `SMTP_PASSWORD`; nenhum valor secreto foi aberto ou copiado.
...
- O widget `/static/support-chat.js` e CSS são carregados na landing e no shell autenticado. A API limita requisições, sanitiza a entrada, usa Gemini para classificar temas permitidos e responde com conteúdo revisado; o cliente usa `textContent`. Em 21/09, perguntas rápidas e respostas foram confirmadas em produção. O MX Zoho já resolve; a entrega à caixa e respostas de e-mail pelo aplicativo seguem sem teste por falta de `SMTP_PASSWORD`.
- Registro histórico anterior ao último save do Render, em 21/09: a auditoria de nomes encontrou apenas `SMTP_PORT` e `SMTP_USE_TLS`; nenhum segredo foi aberto nem copiado. O save posterior adicionou `SMTP_HOST`, `SMTP_FROM_EMAIL` e `SMTP_USERNAME`; falta a chave como `SMTP_PASSWORD`.
...
- O checkout grava o e-mail do pagador e o webhook confirmado tenta enviar um recibo transacional SMTP; o estado `SENT`, `FAILED` ou `SKIPPED` evita duplicidade e permite retry idempotente. O envio exige uma configuração SMTP completa; em 21/09, no Render, falta `SMTP_PASSWORD` e a confirmação de entrega real.
...
| E-mails de Customer Success | Lembrete após 7 dias e resumo opcional ao registrar entrevista implementados com opt-in, outbox idempotente, retentativas e exclusão/portabilidade LGPD; envio real aguarda `SMTP_PASSWORD`. Boas-vindas duplicaria a confirmação do Supabase; outras etapas continuam manuais e expiração aguarda uma data confiável | **70%** | **P1.19**, sem duplicar o escopo de entrega paga |
| Entrega paga e histórico de documentos | Webhook enfileira geração no servidor; estado, lease, tentativas e retry são duráveis; os DOCX ficam em biblioteca owner-scoped, com download, retenção e exclusão. `7bc618a` está Live no Render; host/remetente/usuário/porta/TLS estão configurados; faltam `SMTP_PASSWORD` e comprovar uma entrega real | **92%** | **P1.10 — concluir após inserir chave e testar SMTP** |
...
11. **Entrega pós-pagamento — P1.10:** `7bc618a` publica geração durável no servidor após webhook confirmado, além de recibo, persistência privada, histórico, download owner-scoped, e-mail dos dois DOCX, retenção, exclusão LGPD e retry de geração. Host, remetente, usuário, porta e TLS estão salvos no Render; resta o proprietário inserir `SMTP_PASSWORD` e confirmar uma entrega real em Gmail/Outlook.
...
- Registro histórico: nesta inspeção anterior, o ambiente não tinha variáveis `SMTP_*`. Atualização posterior de 21/09/2026: host, remetente, usuário, porta e TLS foram salvos; permanecem pendentes `SMTP_PASSWORD` e o teste de entrega (**P1.10**).
...
| Recibo e documentos por e-mail | Recibo idempotente; os dois DOCX são anexados somente após geração, com destinatário confirmado, outbox, retry e biblioteca protegida | **P1.10 entregue no código**; falta `SMTP_PASSWORD` e teste real |

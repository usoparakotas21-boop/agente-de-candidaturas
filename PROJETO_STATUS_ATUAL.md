...
**Git e produção:** a marca Candidatura Certa, o favicon, os contatos públicos, o chat de suporte e o Copiloto Chrome 1.0.0 estão publicados. O commit `10a280f` está **Live** no Render e inclui a melhoria de transparência do alerta de expiração; a correção de transporte SMTP está em `9d29d60`, e a matriz de portais/ZIP estável continua publicada desde `1394cd6`. A extensão faz análise contextual da vaga, deixa o Gemini desligado até a pessoa escolher, explica quais dados serão usados e oferece cópia rápida. O pacote inclui ícones finais, arquivo ZIP atualizado e instruções de instalação manual; a publicação na Chrome Web Store continua pendente. A página Perfil explica a instalação em etapas e oferece ajuda.
...
**E-mail de contato:** o endereço foi publicado na interface; Cloudflare contém MX do Zoho (`mx.zoho.com`, `mx2.zoho.com`, `mx3.zoho.com`) e o resolvedor público 1.1.1.1 confirma os três registros. O SPF publicado autoriza o relay Brevo (`v=spf1 include:spf.brevo.com ~all`). No Render, `SMTP_PASSWORD` aparece salva com os demais campos do relay; o valor segue mascarado. Após testes controlados expirarem na fase de conexão usando STARTTLS e depois SSL implícito, a configuração operacional está em `SMTP_PORT=465` e `SMTP_USE_TLS=false` (deploy de ambiente `dep-daoqc6v40ujc73898oug` Live), e o código seleciona `SMTP_SSL` automaticamente nessa porta. O commit `3bda5bd` está Live no Render e também prepara o fallback HTTPS pela API da Brevo, opt-in via `BREVO_API_KEY`. A nova tentativa controlada também expirou em `smtp_stage=connect` (`TimeoutError`, sem código SMTP), antes da autenticação; portanto não há evidência de senha inválida, e-mail confirmado ou recibo entregue. A sessão autenticada mostra o documento pago na biblioteca. O Render Shell não está disponível no plano Free.
...
| P1.22 — prioridade máxima do produto | Candidatura automática por canal autorizado | **40% (estimativa)** | Perfil, análise, documentos direcionados, histórico e Copiloto Chrome 1.0.0 autenticado, com botão persistente opcional por domínio, preparação, análise contextual da vaga, consentimento explicado em linguagem simples, sugestões Gemini com opt-in, cópia rápida e preenchimento consentido estão implementados e publicados. A versão estável 1.0.0 está Live no Render e distribuída por ZIP para instalação manual, agora com seletores para seis portais autorizados; a Chrome Web Store ainda não publicou o pacote. **Submissão automática continua em 0%:** não há conector oficial autorizado. Faltam API/convênio e autorização do empregador/ATS, além de consentimento por vaga/destino. O commit `58f973b` está Live e corrigiu os limites de taxa que causavam HTTP 500 nas rotas de reenvio, prévia e envio por e-mail. O commit `3bda5bd` está Live e adiciona suporte ao relay SMTP com TLS implícito e fallback opcional pela API HTTPS da Brevo; o complemento não envia formulários nem contorna CAPTCHAs ou proteções; para cada portal, o autofill só pode ser usado quando permitido. |
| P1.10 | Geração paga, recibo e entrega por e-mail | **92%** | `SMTP_PASSWORD` está na lista de variáveis salvas no Render e o Brevo mostra uma chave SMTP ativa, com servidor e porta configurados. O commit `58f973b` corrigiu o erro HTTP 500 do limite de taxa; o reenvio respondeu HTTP 200, mas a primeira tentativa expirou antes da autenticação, na conexão STARTTLS. O commit `3bda5bd` está Live e adicionou `SMTP_SSL` automático na porta 465 e fallback HTTPS pela API da Brevo. O commit `caa00a5` torna SMTP completo prioritário sobre qualquer `BREVO_API_KEY` residual e registra separadamente conexão, autenticação, envio e API; 67 testes Python focados passaram. A nova tentativa controlada também expirou em `smtp_stage=connect` (`TimeoutError`, sem código SMTP), portanto a falha ocorre antes da autenticação. O ambiente Live usa `SMTP_PORT=465` e `SMTP_USE_TLS=false`; a confirmação de recibo/documentos na caixa continua bloqueada pela conectividade do relay até que `BREVO_API_KEY` seja configurada no Render. O Render Shell não está disponível no plano Free. |
| P1 | Assinaturas mensais Start e Pro | **90% (estimativa)** | Start R$ 34,90/mês e Pro R$ 99/mês, cobrança recorrente, webhook e cancelamento têm código e 9 testes locais; falta validar uma assinatura real e seu cancelamento. Nenhuma cobrança foi iniciada nesta rodada. |
| P1 | Bônus e publicação do e-book Hackeando DISC | **90% (estimativa)** | O arquivo está rastreado no Git, a rota limita o download ao Pro e há testes de autorização/arquivo ausente; falta confirmar o download em produção com uma conta Pro. A publicação editorial/distribuição do livro continua no lembrete de 22/09. |
...
| P1.19 | E-mails de ciclo de vida | **85% (estimativa)** | Follow-up após 7 dias, avisos de entrevista e alertas de expiração têm opt-in explícito, outbox durável, retries, agrupamento e retenção de 60 dias. O novo dispatcher usa apenas `JobPosting.validThrough`, revalida consentimento/data antes do envio, deduplica por vaga e prazo e mantém a outbox server-only no RLS. A tela agora explica que o alerta de expiração está “Em breve”, aguardando validação de fontes oficiais e configuração do envio. Resta validar a entrega real numa caixa, agora que `SMTP_PASSWORD` aparece salva no Render. |
| P1.11 | Fechamento dos textos legais | **85% (estimativa)** | A política de privacidade agora nomeia os prestadores efetivamente usados, explica a transferência para IA e registra retenção de conteúdo bruto/documentos por 60 dias e backups cifrados por 30 dias. Falta a identificação legal do controlador (nome civil ou razão social e dados cadastrais aplicáveis), que não pode ser inferida da marca. |
| P1.18 | Suporte ao cliente | **90% (estimativa)** | Central de ajuda, WhatsApps e chat flutuante público/autenticado estão publicados. A base inclui dúvidas comuns, quatro sugestões rápidas e link para `/ajuda`; respostas conhecidas têm fallback local se Gemini retornar `unknown`. O launcher agora sobe quando o rodapé público ou autenticado entra na tela; `8e38a0a` está Live e o link do WhatsApp foi conferido visivelmente sem obstrução. Os 43 testes focados de chat e shell passaram. Os MX Zoho já resolvem publicamente; falta validar a entrega real das respostas/notificações pelo app e confirmar o canal ponta a ponta. |
...
| E-mails de Customer Success | Lembrete após 7 dias e resumo opcional ao registrar entrevista implementados com opt-in, outbox idempotente, retentativas e exclusão/portabilidade LGPD; configuração SMTP está salva, resta validar entrega real depois da confirmação do endereço da conta. Boas-vindas duplicaria a confirmação do Supabase; outras etapas continuam manuais e expiração aguarda uma data confiável | **70%** | **P1.19**, sem duplicar o escopo de entrega paga |

...
**E-mail de contato:** o endereço foi publicado na interface; os registros DNS atuais mostram MX nulo (`.`) e SPF `-all`, sem recebimento configurado. No Render, `SMTP_PORT` e `SMTP_USE_TLS` existem, mas `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME` e `SMTP_PASSWORD` estão ausentes. Não considerar recebimento nem envio operacional até configurar o provedor/caixa, os registros DNS e testar.
...
| P1.10 | Geração paga, recibo e entrega por e-mail | **92%** | `SMTP_PORT` e `SMTP_USE_TLS` existem no Render; faltam `SMTP_HOST`, `SMTP_FROM_EMAIL`, `SMTP_USERNAME` e `SMTP_PASSWORD`, além da prova de recibo + currículo + carta em uma caixa confirmada. |
